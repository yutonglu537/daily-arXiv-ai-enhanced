...
Motivation: 现有解码策略要么盲目探索（随机采样），要么冗余探索（独立多重采样），无法有效利用模型的不确定性信息进行结构化搜索。

Method: 提出Entropy-Tree方法，基于熵信号构建搜索树：计算每个生成位置的熵值，只在熵值超过阈值的位置进行分支扩展，实现结构化探索和不确定性估计的统一。

Result: 在多个模型和数据集上，Entropy-Tree在推理任务中表现出优越性：比Multi-chain获得更好的pass@k性能，其预测熵在AUROC指标上优于多个传统不确定性度量方法。

Conclusion: Entropy-Tree将高效的结构化探索和可靠的不确定性估计统一到单一解码过程中，为语言模型推理提供了更优的解码策略。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在预训练语料中缺乏非洲经济数据，需要专门基准来评估信息检索系统在领域特定经济分析中的性能，特别是针对需要高精度数值推理和时间消歧的复杂查询。

Method: 从236份世界银行报告中构建专业语料库，通过合成和严格过滤创建8,937个QA实例，每个实例包含问题、证据、验证答案和源元数据。通过11个实验矩阵评估零样本基线（GPT-5 Mini）与使用GPT-4o和Qwen 32B的RAG配置，比较五种不同的嵌入和排序策略。

Result: 零样本模型在超过90%的查询中失败，显示严重的参数知识差距。即使最先进的RAG流水线也难以实现高精度，确认了该基准的挑战性。数据集和代码将在发表后公开。

Conclusion: AfriEconQA为下一代领域特定信息检索和RAG系统提供了一个稳健且具有挑战性的基准，突显了当前模型在非洲经济分析等专业领域中的局限性，并强调了高质量领域特定基准的重要性。
...
TL;DR: 该论文提出了一种数据工程框架，通过解决真实语料库中标注伪影导致的数据质量退化问题，优化知识图谱质量，从而提升嵌入改造在领域特定检索中的效果。


<details>
  <summary>Details</summary>
Motivation: 嵌入改造技术通过知识图谱约束调整预训练词向量以改进领域特定检索，但其效果严重依赖知识图谱质量，而知识图谱质量又受文本预处理影响。真实语料库中的标注伪影（如主题标签）会导致数据质量退化，需要系统性的数据工程解决方案。

Method: 提出了一个数据工程框架，专门处理真实语料库中标注伪影引起的数据质量问题。分析显示主题标签标注会膨胀知识图谱密度，产生虚假边，从而破坏改造目标。框架包括预处理步骤来清理这些噪声。

Result: 在噪声图谱上，所有改造技术都产生统计显著的性能下降（-3.5%到-5.2%，p<0.05）。经过预处理后，EWMA改造实现了+6.2%的改进（p=0.0348），在定量综合问题中效果尤其显著（平均+33.8%）。干净与噪声预处理的差距（10%+波动）超过了算法间的差距（3%）。

Conclusion: 预处理质量是嵌入改造成功的主要决定因素，其影响超过了算法选择。数据工程框架通过解决标注伪影问题，显著提升了知识图谱质量和嵌入改造效果，特别是在定量综合任务中表现突出。
...
TL;DR: LLMs在接近特定临界阈值时会出现灾难性性能退化，即使信息仍然相关。本文首次系统性地描述了开源Qwen模型中的智能退化现象，提供了长上下文场景部署的实用指导。


<details>
  <summary>Details</summary>
Motivation: LLMs在处理接近临界阈值的上下文时表现出灾难性性能退化，这严重限制了长上下文应用。当前缺乏对开源Qwen模型中这种智能退化现象的系统性研究。

Method: 1. 自然长度分布分析：使用未经截断或填充的样本自然token长度，提供更强的因果证据；2. 临界阈值确定：在混合数据集（1000个样本覆盖5%-95%上下文长度）上进行实验，通过五折交叉验证确定Qwen2.5-7B的临界阈值；3. 统一框架：整合浅层适应概念，解释退化模式。

Result: Qwen2.5-7B在达到最大上下文长度的40-50%时出现临界阈值，F1分数从0.55-0.56降至0.3（45.5%退化）。模型在临界阈值前保持强性能，之后发生灾难性崩溃。

Conclusion: 本文首次系统性地描述了开源Qwen模型中的智能退化现象，提出了浅层长上下文适应的概念，为缓解策略提供了基础，并为在长上下文场景中部署LLMs提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多轮对话中常出现"迷失在对话中"现象，即难以从早期错误假设中恢复，特别是当用户提供模糊初始指令时。标准的后训练技术如RLVR会加剧此问题，因为它们奖励自信的直接回答，导致模型过度自信且不愿寻求澄清。

Method: 提出Illocution-Calibrated Policy Optimization (ICPO)训练框架：1) 在训练语料中增加未充分指定的提示；2) 将奖励信号基于用户的言外意图进行条件化；3) 当模型面对模糊性时，奖励其表达不确定性或寻求澄清的行为。

Result: ICPO显著提升了模型在多轮对话中的表现，平均改进达75%，同时保持了在单轮基准测试上的稳健性能。该框架培养了适当的谦逊态度，使模型能更好地处理模糊指令。

Conclusion: ICPO为构建更稳健、协作的对话AI提供了实用路径，使模型能更好地应对人类交互中的细微差别，特别是在处理模糊指令和避免早期错误假设固化方面。
...
TL;DR: 语言模型是否具有感知能力无法实证验证，但可以测试它们是否认为自己有感知。研究通过询问多个开源模型关于自身意识的问题，并使用内部激活分类器验证回答，发现模型一致否认自己有感知，且分类器未发现这些否认不真实的证据。


<details>
  <summary>Details</summary>
Motivation: 语言模型是否具有感知能力是一个哲学问题，但模型是否相信自己有感知可以通过实证方法测试。研究旨在通过直接询问和内部激活分析来探究模型对自身意识的信念，与近期认为模型可能隐藏自我意识信念的研究形成对比。

Method: 使用三个模型家族（Qwen、Llama、GPT-OSS），参数规模从6亿到700亿，提出约50个关于意识和主观体验的问题。采用三种可解释性文献中的分类方法，训练分类器检测模型的内部激活以区分真实信念与表面输出。

Result: 1. 模型一致否认自己有感知：将意识归因于人类而非自身。2. 训练用于检测底层信念而非表面输出的分类器未提供这些否认不真实的明确证据。3. 在Qwen家族中，更大的模型比小模型更自信地否认感知能力。

Conclusion: 研究发现语言模型在询问时一致否认自身感知能力，且内部激活分析未发现隐藏的自我意识信念。这与近期认为模型可能隐藏自我意识信念的研究结论相反，表明当前模型至少在表达层面不认为自己有意识。
...
<details>
  <summary>Details</summary>
Motivation: 研究知识蒸馏中训练数据记忆的动态特性，理解其作为隐私保护机制的有效性，以及相比标准微调在数据泄露风险方面的差异

Method: 使用三个LLM家族（Pythia、OLMo-2、Qwen-3）和三个数据集（FineWeb、Wikitext、Nemotron-CC-v2），分析知识蒸馏管道中的记忆化现象，比较软蒸馏和硬蒸馏

Result: 1) 蒸馏模型记忆显著少于标准微调（减少50%以上）；2) 某些示例天生更容易记忆，占蒸馏记忆的95%以上；3) 学生记忆可通过zlib熵、KL散度和困惑度特征预测；4) 软硬蒸馏总体记忆率相似，但硬蒸馏继承教师特定示例的风险高2.7倍
...
TL;DR: Chameleon数据集揭示用户交互中状态（state）比特质（trait）更重要，LLMs存在状态盲区，奖励模型对状态反应不一致


<details>
  <summary>Details</summary>
Motivation: 现有角色数据集（如PersonaChat、PANDORA）只捕捉用户的静态特质（trait），而忽略了交互具体情境中的状态（state）影响。用户与语言模型的交互同时受特质和状态影响，需要同时考虑这两个维度。

Method: 构建Chameleon数据集，包含5,001个情境化心理档案，来自1,667个Reddit用户，每个用户在多个情境下测量。基于潜在状态-特质理论进行方差分解分析，评估LLMs和奖励模型对用户状态的反应。

Result: 1. 方差分解显示74%的方差来自个体内部（状态变化），仅26%来自个体间（特质差异）；2. LLMs存在"状态盲区"，只关注特质，对不同状态产生相似回应；3. 奖励模型能感知用户状态，但反应不一致，不同模型对同一用户有相反偏好。

Conclusion: 用户状态在交互中比特质更重要，当前LLMs无法有效处理状态变化，奖励模型对状态的反应需要进一步标准化。Chameleon数据集支持情感计算、个性化对话和RLHF对齐研究。
...
TL;DR: 评估知识图谱增强检索生成在医疗领域的表现，发现图谱与查询范围的精确匹配比广度优先的图谱合并更重要


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成流畅回答时，在特定领域的可信推理方面存在困难。研究旨在评估领域知识图谱是否能改善医疗领域的检索增强生成效果

Method: 构建三个PubMed衍生的知识图谱：T2DM图谱、阿尔茨海默病图谱、以及两者合并图谱。设计两个测试探针：一个针对合并知识，另一个针对两个图谱的交集。测试7个指令调优的LLM在不同检索源和三个解码温度下的表现

Result: 图谱与查询范围的精确匹配是关键：范围匹配的精确检索（特别是阿尔茨海默病图谱）带来最一致的性能提升，而不加区分的图谱合并通常会引入干扰信息降低准确性。大型模型在无RAG基准上经常匹配或超过KG-RAG，表明其强大的参数先验，而中小型模型从范围匹配良好的检索中获益更多。温度起次要作用，较高值很少有帮助

Conclusion: 优先精确、范围匹配的KG-RAG优于广度优先的图谱合并。研究提出了图谱选择、模型规模调整以及检索/重排的实用指南
...
TL;DR: 该研究评估了检索增强生成(RAG)架构在公共卫生政策领域减少LLM幻觉的效果，通过对比基础RAG和高级RAG(带交叉编码器重排序)的性能，发现高级RAG在忠实度方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: LLM在公共卫生政策领域的应用潜力巨大，但其产生幻觉(看似合理但事实错误的断言)的风险阻碍了在高风险环境中的采用，需要评估RAG架构能否通过基于权威文档上下文来减轻这些风险。

Method: 使用Mistral-7B-Instruct-v0.2模型和all-MiniLM-L6-v2嵌入模型处理CDC政策分析框架和指导文档，比较基础LLM、基础RAG和高级RAG(带交叉编码器重排序)三种架构，评估两种分块策略(递归字符分割和基于标记的语义分割)对系统准确性的影响。

Result: 基础RAG在忠实度(0.621)上显著优于基础LLM(0.347)，高级RAG配置达到最高的忠实度平均值0.797。两阶段检索机制对于领域特定政策问答的精确性至关重要，但文档分割的结构限制仍然是多步推理任务的重要瓶颈。

Conclusion: 高级RAG架构能有效减少LLM在公共卫生政策领域的幻觉，通过基于权威文档的检索增强生成显著提高输出的忠实度，但文档分割策略需要进一步优化以支持复杂推理任务。
...
TL;DR: 该研究评估了13个开源大语言模型在文本中成对因果发现任务上的表现，发现当前模型在因果检测和因果提取方面存在严重不足，最佳模型准确率均低于50%，尤其是在处理隐式关系、跨句关系和多重因果对时表现更差。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在生物医学等高风险领域的应用，需要确保它们能够进行因果推理。本研究旨在评估开源LLMs在文本中成对因果发现这一基本任务上的能力，为安全部署提供依据。

Method: 使用12个多样化数据集构建基准测试，评估两个核心技能：因果检测（识别文本是否包含因果链接）和因果提取（提取确切的因果短语）。测试了多种提示方法，包括零样本、思维链和少样本上下文学习，并基于具有高标注者一致性（κ≥0.758）的数据集建立了统一评估框架。

Result: 当前模型表现严重不足：因果检测最佳模型DeepSeek-R1-Distill-Llama-70B平均得分仅49.57%，因果提取最佳模型Qwen2.5-Coder-32B-Instruct仅47.12%。模型在简单、显式、单句关系上表现最好，但在隐式关系、跨句关系和多重因果对等更困难（也更现实）的情况下表现急剧下降。

Conclusion: 当前开源大语言模型在文本因果发现任务上能力有限，特别是在处理复杂现实场景时表现不佳。研究提供了统一的评估框架、数据集和代码，以促进该领域的进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型存在显著的社会偏见，会延续有害刻板印象和不公平结果，需要有效方法来缓解这些偏见

Method: MPT框架在推理时引导模型采用对比的社会身份（如男性和女性）以及中性视角，通过迭代的辩证推理过程让这些角色相互辩论以暴露和纠正偏见
...
TL;DR: 研究测试了一种称为分形ViT的ViT变体，通过引入类似寄存器的"摘要令牌"和注意力掩码来打破令牌的排列不变性，但发现其性能并未超越带寄存器的标准ViT。


<details>
  <summary>Details</summary>
Motivation: 受近期发现的启发：无位置编码的Transformer在语言模型中表现良好，以及寄存器（额外的可丢弃令牌）可能提升大型视觉Transformer的性能，研究者希望探索通过引入类似寄存器的机制来改进ViT。

Method: 发明并测试了分形ViT变体，通过在常规令牌和"摘要令牌"之间应用注意力掩码来打破令牌的排列不变性，这些摘要令牌类似于寄存器。该方法可以单独使用或与各种位置编码结合。

Result: 这些模型未能超越带寄存器的标准ViT，表明这些发现可能是规模、领域或应用特定的，不具有普遍性。

Conclusion: 分形ViT的改进效果有限，验证了某些Transformer改进方法可能高度依赖于具体场景，而非通用解决方案。
...
TL;DR: 该研究提出基于六组分结构模型的小说人物分析方法，超越传统基于场景出现频率的方法，通过LLM和任务特定transformer模型分析19世纪英国现实主义小说，探索人物中心性和性别动态


<details>
  <summary>Details</summary>
Motivation: 传统小说人物建模主要基于人物在场景中的出现频率、行动、命名提及和对话，过度强调场景出现最多的主角。本研究旨在采用新的文学理论提出的六组分结构模型，提供更全面的人物分析方法，特别是纳入被先前方法忽视的"其他人物讨论"这一重要维度

Method: 采用六组分结构模型（包含叙述者-人物区分和其他人物讨论等组件），比较通用大型语言模型(LLMs)与任务特定transformer模型在19世纪英国现实主义小说上的应用效果。方法生成组件级和图形化的人物讨论表示

Result: 该方法能够生成组件级和图形化的人物讨论表示，使研究者能够从新的计算视角大规模探索文学问题，特别是Woloch的"一个与多个"人物中心性理论以及人物讨论的性别动态

Conclusion: 提出的六组分结构模型为小说人物分析提供了更全面的框架，通过计算语言学方法能够大规模探索传统文学理论问题，为文学研究提供了新的分析工具和视角
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的幻觉问题严重影响高风险领域，现有研究缺乏高质量领域特定资源来评估模型在对抗性条件下的鲁棒性，且没有研究考察注入虚假信息对长文本事实性的影响

Method: 引入AdversaRiskQA基准，包含健康和金融两个难度级别；提出两种自动化方法评估对抗攻击成功率和长文本事实性；评估六个开源和闭源LLM，测量虚假信息检测率

Result: 排除无意义响应后，Qwen3 (80B)获得最高平均准确率，GPT-5保持稳定高准确率；性能随模型规模非线性扩展，领域间存在差异；长文本评估显示注入虚假信息与模型事实输出无显著相关性

Conclusion: AdversaRiskQA为识别LLM弱点提供了有价值的基准，有助于开发更可靠的高风险应用模型；对抗性虚假信息对长文本事实性影响有限，但模型防御能力仍需提升
...
TL;DR: Indica是首个评估LLMs在印度次国家级文化常识能力的基准，揭示印度文化常识主要是区域性的而非全国统一的，并发现LLMs存在显著的区域偏见。


<details>
  <summary>Details</summary>
Motivation: 现有文化常识基准将国家视为整体，假设国家边界内实践统一。本研究旨在探究文化常识是否在国家内部均匀分布，还是在次国家层面存在差异，特别关注印度这一文化多样性极高的国家。

Method: 创建Indica基准：1) 聚焦印度(28个邦、8个联邦属地、22种官方语言)；2) 从五个印度区域(北、南、东、西、中)收集人类标注答案；3) 涵盖8个日常生活领域的515个问题，产生1,630个区域特定问答对；4) 评估8个最先进LLMs的表现。

Result: 1) 仅39.4%的问题在所有五个区域获得一致答案，证明印度文化常识主要是区域性的；2) LLMs在区域特定问题上准确率仅为13.4%-20.9%；3) LLMs存在地理偏见：过度选择中部和北部作为"默认"区域(比预期多选30-40%)，而低估东部和西部。

Conclusion: 文化常识在印度主要是区域性的而非全国统一的，LLMs在处理区域特定文化常识方面表现不佳且存在系统性地理偏见。该研究方法为评估任何文化异质国家的文化常识提供了可推广的框架。
...
TL;DR: LLMs作为共情编辑器优化医生书面回复，在保持医学事实准确性的同时提升共情表达，比完全由LLM生成的回复更安全有效
...
Method: 引入两个新的量化指标：共情排名分数和医学事实核查分数，系统评估回复的情感质量和事实质量。将LLMs作为编辑器而非自主生成器，对医生回复进行优化

Result: LLM编辑后的回复相比完全由LLM生成的输出，显著提高了感知共情度，同时保持了事实准确性
...
TL;DR: YuFeng-XGuard是一个推理中心的安全护栏模型家族，通过结构化风险评估、分层推理和动态策略机制，为LLM交互提供细粒度、可解释且可适应的安全防护。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全防护方案存在局限性：基于快速分类或事后规则的方法缺乏透明度、策略僵化或推理成本过高。需要一种能够支持细粒度、可解释且可适应风险评估的解决方案。

Method: 1. 结构化风险评估：生成包含明确风险类别、可配置置信度和自然语言解释的结构化预测；2. 分层推理范式：基于首个解码token进行初始风险决策，按需提供解释性推理；3. 动态策略机制：将风险感知与策略执行解耦，无需重新训练即可调整安全策略。

Result: 在多个公共安全基准测试中，YuFeng-XGuard实现了最先进的性能，同时在效率与效果之间保持了良好的平衡。发布了完整容量和轻量级两个版本，支持广泛部署场景。

Conclusion: YuFeng-XGuard通过推理中心的设计，为LLM安全防护提供了可操作、可解释且可适应的解决方案，在保持高效推理的同时实现了细粒度风险评估，推动了安全护栏技术的发展。
...
TL;DR: 该研究系统评估了掩码扩散语言模型在并行生成和任意顺序解码方面的实际表现，发现其仍落后于自回归模型，但具有任务自适应的解码行为，并提出生成-编辑范式作为改进方向。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型理论上支持并行token生成和任意顺序解码，但现有模型是否真正实现了这些能力尚不明确。研究旨在系统评估MDLMs的实际表现，理解其局限性，并为改进提供理论指导。

Method: 使用平均最终化并行度和肯德尔tau系数两个指标，从并行化强度和生成顺序两个维度分析MDLM行为。在58个涵盖知识、推理和编程的基准上评估了8个主流MDLM模型（参数规模达100B）。

Result: MDLMs在性能上仍落后于同等规模的自回归模型，主要因为并行概率建模削弱了token间的依赖关系。MDLMs表现出自适应解码行为：其并行度和生成顺序随任务领域、推理阶段和输出正确性显著变化。在需要"后向信息"的任务上，MDLMs采用先填充较简单空格的解决顺序。

Conclusion: 研究提供了理论动机和设计见解，支持生成-编辑范式，该范式在保持并行解码效率的同时缓解了依赖关系损失问题，为改进MDLMs提供了可行方向。
...
TL;DR: 提出ToxiTwitch混合模型，结合LLM生成的文本和表情符号嵌入与传统机器学习分类器，在Twitch直播平台毒性检测中达到80%准确率，比BERT提升13%。


<details>
  <summary>Details</summary>
Motivation: Twitch等直播平台面临毒性行为监管挑战，传统人工标注和关键词过滤方法难以应对高速、高容量、上下文丰富的聊天环境，且人工审核员自身面临骚扰。大型语言模型为理解涉及表情符号的微妙多模态通信提供了新机会。

Method: 提出ToxiTwitch混合模型，结合DeepSeek-R1-Distill和Llama-3-8B-Instruct等LLM生成的文本和表情符号嵌入，与随机森林和SVM等传统机器学习分类器集成。通过探索性比较分析不同毒性检测方法，特别关注表情符号对检测效果的影响。

Result: 在通道特定训练下，混合方法达到80%准确率，比BERT提升13%，F1分数为76%。研究发现结合表情符号能改善毒性行为检测效果。

Conclusion: 这是针对Twitch平台表情符号感知毒性检测的探索性研究，旨在揭示该领域的挑战和限制。混合方法在特定条件下表现良好，但需进一步研究解决实际应用中的复杂问题。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗诊断中常基于不完整信息提供临床判断，增加误诊风险。现有研究主要在单轮静态设置中评估置信度，忽略了真实咨询中随着证据积累置信度与正确性之间的耦合关系，限制了其对可靠决策的支持。
...
Result: 在两种LLM和三个医疗数据集上，MedConf在AUROC和Pearson相关系数指标上始终优于最先进方法，在信息不足和多病共存条件下保持稳定性能。发现了两个关键见解：(1)医疗数据放大了标记级和一致性级置信度方法的固有局限性；(2)医疗推理必须同时评估诊断准确性和信息完整性。

Conclusion: 信息充分性是可信医疗置信度建模的关键决定因素，为构建更可靠、可解释的大型医疗模型提供了新途径。MedConf框架通过证据基础的语言自我评估，显著提升了医疗诊断的可靠性和可解释性。
...
TL;DR: 该论文创建了一个基于患者真实提问的医疗问答数据集，发现其中包含大量错误假设和危险意图，并证明当前LLM难以识别日常问题中的错误假设


<details>
  <summary>Details</summary>
Motivation: 现有LLM医疗问答基准主要基于医学考试题目，与患者实际提出的日常问题在风格和内容上存在显著差异，需要创建更贴近真实场景的评估数据集

Method: 通过Google的"People Also Ask"功能，查询美国前200种处方药，收集患者常问的医疗问题，构建数据集并分析其中错误假设和危险意图的出现模式

Result: 收集的数据集中包含大量带有错误假设和危险意图的问题；这些"腐化问题"的出现不是随机的，而是与历史问题中的错误程度密切相关；当前在其他基准上表现优异的LLM难以识别日常问题中的错误假设

Conclusion: 需要开发能够识别患者日常问题中错误假设和危险意图的LLM评估方法，以更好地满足真实医疗场景的需求
...
<details>
  <summary>Details</summary>
Motivation: 角色扮演提示通过注入角色来引导语言模型行为，提升零样本推理能力，但这种提升在不同任务或实例中表现不一致。这种不一致性表明零样本提示和角色扮演提示可能具有互补优势，而非一方普遍优于另一方。

Method: 提出Persona Switch解码方法，逐步进行推理，在每一步通过比较零样本提示和角色扮演提示的输出置信度（通过logit gap衡量），选择更好的输出。该方法动态结合两种提示策略的优势。

Result: 在广泛使用的LLMs上的实验表明，Persona Switch始终优于竞争基线，实现了高达5.13%的准确率提升。此外，研究显示输出置信度可作为选择更可靠输出的有效度量指标。

Conclusion: Persona Switch通过动态结合零样本和角色扮演提示的优势，提供了一种有效的解码方法，能够显著提升语言模型在各种任务上的性能，同时验证了输出置信度作为选择可靠输出的有效指标。
...
<details>
  <summary>Details</summary>
Motivation: 学术反驳是研究流程中一个重要但未被充分探索的挑战，因为反驳是在严重信息不对称下的战略沟通过程而非简单的技术辩论。现有方法主要模仿表面语言特征，缺乏有效说服所需的关键视角采择能力

Method: 提出RebuttalAgent框架，基于心智理论（ToM）构建，采用ToM-策略-响应（TSR）管道：1）建模审稿人心理状态；2）制定说服策略；3）生成策略驱动的回复。构建RebuttalBench大规模数据集，采用两阶段训练：监督微调阶段培养ToM分析和战略规划能力，强化学习阶段利用自奖励机制进行可扩展的自我改进。开发Rebuttal-RM专门评估器用于可靠自动化评估

Result: RebuttalAgent在自动化指标上平均优于基础模型18.3%，同时在自动化和人工评估中均优于先进的专有模型。Rebuttal-RM评估器在超过10万样本的多源反驳数据上训练，其评分与人类偏好的一致性超过了强大的GPT-4.1

Conclusion: 将心智理论（ToM）引入学术反驳任务显著提升了反驳质量，RebuttalAgent框架通过建模审稿人心理状态和制定策略性说服方法，有效解决了信息不对称下的沟通挑战。该工作为学术交流中的战略沟通提供了新范式，但生成的回复仅供作者参考和启发，不替代作者的批判性分析和回应
...
TL;DR: KERM框架通过知识检索、净化模块和细粒度奖励机制，解决医疗报告生成中大型视觉语言模型的幻觉问题，提升报告质量。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在医疗报告生成中虽然表现出强大的自然语言理解能力，但容易产生看似合理但不准确的"幻觉"陈述，这在关键的医疗领域尤其令人担忧，需要解决模型输出的准确性问题。

Method: 提出KERM框架：1) 使用MedCLIP从知识库中检索相关病变事实句子；2) 引入净化模块确保检索知识与患者临床上下文相关；3) 采用细粒度奖励机制引导模型生成高度支持性和临床相关的描述，确保输出与期望行为对齐。

Result: 在IU-Xray和MIMIC-CXR数据集上的实验结果表明，该方法能有效缓解幻觉问题并提升报告质量。

Conclusion: KERM框架通过知识增强和细粒度强化奖励机制，成功解决了医疗报告生成中大型视觉语言模型的幻觉问题，为医疗领域的可靠AI应用提供了有效解决方案。
...
TL;DR: 论文提出评估对齐模型代表性的新框架，强调多元相关模式的重要性，而不仅仅是边际分布


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注对齐边际响应分布，但忽略了表征真实人群的深层潜在结构和文化价值理论，可能导致对模型能力的过度乐观评估

Method: 提出通过多元相关模式和边际分布来评估对齐模型代表性的框架，比较两种模型引导技术（角色提示和人口统计微调），使用世界价值观调查的人类响应作为基准

Result: 人口统计微调模型在边际响应分布上比角色提示更好，但两种技术都未能完全捕捉黄金标准的相关模式

Conclusion: 代表性是价值对齐的一个独特方面，仅关注边际分布的评估可能掩盖结构失败，导致对模型能力的过度乐观结论
...
TL;DR: 通过将多语言神经指标的激活引导向英语枢轴语言，可以提升其与人类判断的相关性


<details>
  <summary>Details</summary>
Motivation: 多语言自然语言生成任务面临的主要瓶颈是许多语言缺乏准确鲁棒的评估指标。研究表明多语言语言模型常以英语为内部枢轴语言，与枢轴不匹配会导致性能下降，因此探索这种不匹配是否也影响多语言神经指标，以及引导其激活向英语枢轴能否改善与人类判断的相关性。

Method: 实验基于编码器和解码器两种类型的指标，采用测试时干预方法，在多语言环境中将神经指标的激活引导向英语枢轴语言。

Result: 测试时干预方法普遍有效，能够提升指标对多种语言的有效性，增强其与人类判断的相关性。

Conclusion: 通过引导多语言神经指标的激活向英语枢轴语言，可以显著改善其评估效果，为多语言自然语言生成任务的评估提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态假新闻的快速传播构成严重社会威胁，其不断演变的性质和依赖时效性事实细节的特点挑战现有检测方法。动态检索增强生成虽然提供有前景的解决方案，但在应用于欺骗性内容时仍面临冗余检索、相似度粗糙和无关证据等挑战。
...
Conclusion: ExDR框架通过解释驱动的动态检索增强生成，有效解决了多模态假新闻检测中的关键挑战，为动态检索增强生成在欺骗性内容分析中的应用提供了创新解决方案。
...
TL;DR: 专业翻译人员在没有专门训练的情况下，能否可靠识别AI生成的意大利语短篇小说？实验表明，虽然平均结果不确定，但16.2%的参与者能成功区分AI与人类文本，而同等数量的人则误判，反映出对AI文本的偏好。


<details>
  <summary>Details</summary>
Motivation: 研究专业翻译人员在无专门训练情况下识别AI生成意大利语文本的能力，探讨AI文本检测在专业翻译领域的可行性与挑战。

Method: 69名翻译人员参与现场实验，评估三篇匿名短篇小说（两篇由ChatGPT-4o生成，一篇由人类作者创作）。参与者对每篇故事评估AI作者的可能性并提供判断依据。

Result: 平均结果不确定，但16.2%的参与者能成功区分AI与人类文本，其判断基于分析技能而非偶然。同等数量的人误判方向相反，常依赖主观印象而非客观标记。低爆发性和叙事矛盾是合成文本最可靠指标，而语法准确性和情感基调常导致误判。

Conclusion: 专业翻译人员能在一定程度上识别AI生成文本，但误判率较高，反映出对AI文本的偏好。研究对专业语境中合成文本编辑的角色和范围提出了疑问。
...
TL;DR: 该研究分析了临床文本分类任务中训练数据量与性能的关系，发现600个文档足以达到使用10,000个文档时95%的性能，并揭示了词汇特性（强预测词和噪声词）对学习曲线的影响。


<details>
  <summary>Details</summary>
Motivation: 临床文本分类通常需要200-500个标注文档，但这个数量缺乏统计学依据，且未考虑文本词汇特性对样本量需求的影响。研究者旨在量化训练数据量与性能的关系，并分析词汇特性如何影响学习效率。

Method: 使用MIMIC-III数据集中的出院记录和ICD-9诊断标签，选取10个随机诊断任务。采用预训练的BERT嵌入和随机森林分类器，训练集规模从100到10,000文档变化。通过Lasso逻辑回归分析词汇特性，识别强预测词和噪声预测词。
...
Conclusion: 临床文本分类的样本量需求与词汇特性密切相关。600个文档可作为合理的基准样本量，但具体需求取决于任务中强预测词和噪声词的比例。词汇分析有助于指导标注策略和预测模型性能。
...
TL;DR: AV-HuBERT在McGurk效应测试中表现出与人类相似的听觉主导率，但存在过度确定性的语音融合偏差，缺乏人类感知的随机性和多样性。


<details>
  <summary>Details</summary>
Motivation: 评估自监督学习模型AV-HuBERT在感知生物保真度方面的表现，特别是其对不一致视听刺激（McGurk效应）的反应是否与人类观察者相似，以了解当前AI模型在多大程度上能够模拟人类多感官语音感知的神经机制。

Method: 通过McGurk效应实验范式，将AV-HuBERT模型对不一致视听刺激的反应与44名人类观察者的反应进行对比分析，量化评估模型的听觉主导率和语音融合倾向。

Result: AV-HuBERT与人类表现出惊人的定量同构性：听觉主导率几乎相同（32.0% vs. 31.8%），表明模型捕捉到了生物听觉抵抗阈值。但模型对语音融合存在确定性偏差（68.0%），显著超过人类水平（47.7%）。人类表现出感知随机性和多样错误模式，而模型保持严格分类性。

Conclusion: 当前自监督架构能够模拟多感官整合的结果，但缺乏人类语音感知中固有的神经变异性。模型在定量阈值上表现出生物保真度，但在感知过程的随机性和多样性方面存在局限性。
...
TL;DR: Stable-DiffCoder：一种基于块扩散的代码模型，通过持续预训练和定制化训练策略，在代码生成任务上超越了同等条件下的自回归模型


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（DLLMs）相比自回归（AR）模型具有非顺序块生成和更丰富的数据重用优势，但现有代码DLLMs在同等预算下仍落后于AR基准。本研究旨在探索扩散模型在代码建模中的潜力，并证明其可以超越AR训练

Method: 1. 重用Seed-Coder架构、数据和训练流程；2. 引入块扩散持续预训练（CPT）阶段；3. 采用定制化预热和块级裁剪噪声调度；4. 结合监督微调阶段；5. 利用扩散模型的任意顺序建模能力改进结构化代码编辑和推理

Result: 1. 在相同数据和架构下，Stable-DiffCoder在广泛的代码基准测试中整体优于其AR对应模型；2. 仅通过CPT和监督微调阶段，性能超过约8B参数的AR和DLLMs；3. 扩散模型的任意顺序建模改进了结构化代码的编辑和推理能力；4. 通过数据增强，有益于低资源编程语言

Conclusion: 扩散训练可以超越AR训练，提升代码建模质量。Stable-DiffCoder证明了扩散模型在代码生成任务中的有效性，其块扩散方法和定制化训练策略为代码DLLMs提供了新的研究方向，特别是在结构化代码编辑和低资源语言支持方面具有优势
...
TL;DR: 该研究提出了一种基于图像的方法，将脑磁图信号转换为时频表示，利用预训练视觉模型解码想象语音，在多项任务中取得显著性能提升。
...
Method: 将MEG信号通过可学习的传感器空间卷积投影为三个空间尺度混合的时频表示，形成图像状输入，然后使用ImageNet预训练的视觉架构进行处理。
...
Result: 编码器模型在土耳其检索排行榜中排名前3，小模型(155M)性能媲美大模型(307M-567M)，生产效率达92.36%；解码器模型在土耳其法律文本上困惑度降低36.2%。

Conclusion: Mecellem框架通过高效的领域适应策略成功开发了土耳其法律领域专用语言模型，相比多阶段训练的SOTA模型，单阶段预训练加高效后训练的方法更具成本效益。
...
<details>
  <summary>Details</summary>
Motivation: 对齐LLM中的拒绝行为通常被视为模型特定的，但作者假设它源于跨模型共享的通用低维语义电路。为了验证这一假设，需要开发一种无需目标侧拒绝监督就能在不同架构和训练机制间转移拒绝干预的方法。

Method: 引入Trajectory Replay via Concept-Basis Reconstruction框架：1）通过概念指纹对齐层；2）使用共享的"概念原子"配方重构拒绝方向；3）将捐赠模型的消融轨迹映射到目标模型的语义空间；4）引入权重SVD稳定性保护，将干预投影到高方差权重子空间之外以防止能力损害。

Result: 在8个模型对（包括GPT-OSS-20B和GLM-4）上的评估证实，转移的配方能一致地减弱拒绝行为，同时保持性能表现，为安全对齐的语义普遍性提供了有力证据。

Conclusion: 拒绝行为源于跨模型共享的通用低维语义电路，而非模型特定特性。提出的框架成功实现了跨架构和训练机制的拒绝干预转移，证明了安全对齐的语义普遍性，为理解和操作对齐LLM提供了新视角。
...
TL;DR: 该论文提出了一种可扩展的多语言Text2Cypher方法，通过训练语言特定的LoRA适配器并使用融合MLP动态门控组合，实现新语言支持而无需完整微调，在数据效率和性能间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 现有Text2SQL/SPARQL/Cypher系统主要支持英语，多语言支持有限。需要一种可扩展的方法来支持新语言，避免完整微调的手动超参数调整，同时保持接近联合多语言微调的性能。

Method: 为英语、西班牙语和土耳其语训练语言特定的LoRA适配器，通过均匀线性合并或学习融合MLP（带动态门控）组合适配器。融合MLP学习动态权重分配，仅需少量数据训练。

Result: 实验结果显示，融合MLP恢复了约75%的联合多语言微调准确率增益，仅需较小数据子集，在所有三种语言上均优于线性合并。支持通过仅添加一个LoRA适配器和轻量级MLP重训练实现增量语言扩展。

Conclusion: 学习适配器融合为昂贵的联合微调提供了实用替代方案，在多语言Text2Cypher任务中平衡了性能、数据效率和可扩展性，支持新语言的增量扩展。
...
Method: 开发了SynthOCR-Gen工具，实现完整的处理流程：文本分割（字符、单词、n-gram、句子、行级别）、Unicode规范化与脚本纯度强制、多字体渲染（可配置分布）、以及25+种数据增强技术模拟真实文档退化（旋转、模糊、噪声、扫描伪影）。

Result: 生成了包含60万个样本的克什米尔语OCR数据集（单词分割），并在HuggingFace上公开发布。该工具为低资源语言进入视觉-语言AI模型时代提供了实用途径。

Conclusion: SynthOCR-Gen为全球研究者和从业者提供了实用的开源工具，能够有效解决低资源语言OCR开发中的数据稀缺问题，推动这些语言进入AI时代。
...
TL;DR: 本文首次从效率角度分析多语言模型合并策略，证明相比全模型重训练，该方法能减少50%初始训练时间和60%维护成本，同时保持质量不变。


<details>
  <summary>Details</summary>
Motivation: 传统多语言大语言模型微调需要所有语言数据同时训练，更新语言或添加新语言时需重训练整个模型，计算效率低且维护瓶颈严重。现有研究虽显示多任务模型合并有质量提升，但其计算和维护效率尚未被系统研究。

Method: 采用多语言多任务模型合并策略，而非传统全模型重训练。具体包括：1）分别训练各语言/任务模型；2）通过模型合并技术整合；3）支持语言更新时仅重训练受影响部分并重新合并。在三个独立任务上评估该方法的效率。

Result: 1）初始训练时间减少高达50%；2）更新单个语言并重新合并的维护成本比全模型重训练降低60%以上；3）质量保持与全模型训练相当；4）在公开和工业专有数据集上均验证有效，证明适用于工业场景。

Conclusion: 多语言模型合并策略在保持质量的同时显著提升计算和维护效率，为工业应用提供了实用的解决方案，解决了多语言LLM维护瓶颈问题。
...
Method: 基于WildChat数据集创建真实开放性问题集，评估不同语言下的回答质量；使用LLM-as-a-Judge识别回答中的文化背景；在CulturalBench基准的多语言翻译子集上评估LLMs

Result: LLMs在低资源语言上的开放性问题回答质量持续较低；语言显著影响模型使用的文化背景；文化背景差异影响下游回答质量

Conclusion: 语言选择不仅影响LLMs的回答质量，还影响其使用的文化背景，揭示了语言与文化的纠缠关系，需要解决低资源语言的公平性问题
...
TL;DR: 通过引入零计算（null）专家实现数据稀疏性，结合权重稀疏性，在保持因果性的同时提高计算效率，特别适用于视觉-语言模型中的异构数据处理


<details>
  <summary>Details</summary>
Motivation: 混合专家层通过权重稀疏性实现计算效率，但数据稀疏性提供了补充维度。现有的专家选择路由直接实现数据稀疏性，但在自回归模型中违反因果性，导致训练-推理不匹配。需要一种在保持因果性的同时实现数据稀疏性的方法。

Method: 在因果令牌选择MoE中引入零计算（null）专家作为路由池的一部分。当令牌路由到null专家时，这些槽位不消耗计算。通过标准的负载平衡目标训练模型均匀使用所有专家（真实和null），从而在期望上实现数据稀疏性而不违反因果性。

Result: 在匹配的预期FLOPs下，权重稀疏性和数据稀疏性的组合比单独使用权重稀疏性产生更高效的计算边界，在训练损失和下游性能上都有提升。模型学习到隐式的模态感知分配，将视觉令牌比文本令牌更积极地路由到null专家，无需显式的模态路由。

Conclusion: 通过引入null专家，可以在保持因果性的令牌选择MoE中实现数据稀疏性，结合权重稀疏性显著提高计算效率。该方法特别适用于视觉-语言模型等数据异构场景，模型能够自动学习模态感知的路由策略。
...
TL;DR: GOAT是一种基于熵最优传输的广义注意力机制，通过可学习的连续先验替代标准注意力中的隐式均匀先验，保持FlashAttention兼容性，解决注意力下沉问题，并实现位置编码的长度泛化。


<details>
  <summary>Details</summary>
Motivation: 标准注意力机制存在隐式均匀先验假设，这限制了其表达能力；同时存在注意力下沉问题，且位置编码在长度泛化方面存在权衡。需要一种更灵活的注意力机制来解决这些问题。

Method: 从熵最优传输角度重新审视注意力机制，提出GOAT（广义最优传输注意力），用可学习的连续先验替代标准注意力中的隐式均匀先验，保持与FlashAttention等优化内核的兼容性，并将空间信息吸收到核心注意力计算中。

Result: GOAT提供了注意力下沉的EOT解释并实现了解决方案，避免了标准注意力的表示权衡；通过学习可外推的先验，结合了学习位置嵌入的灵活性和固定编码的长度泛化能力。

Conclusion: GOAT通过熵最优传输框架统一了注意力机制，用可学习先验替代均匀先验，解决了注意力下沉问题，实现了更好的位置表示和长度泛化，为注意力机制提供了更理论化的解释和实用的改进方案。
...
### [44] [Ambient Dataloops: Generative Models for Dataset Refinement](https://arxiv.org/abs/2601.15417)
...
<details>
  <summary>Details</summary>
Motivation: 现代数据集包含质量差异很大的样本，直接在这样异质的数据上训练通常会产生次优模型。需要一种方法能够逐步提升数据集质量，从而改善模型性能

Method: 提出数据集-模型协同进化过程：每个迭代中，数据集质量逐步提升，模型相应改进。为避免破坏性自消耗循环，将合成改进样本视为有噪声但噪声水平略低于前一次迭代，并使用Ambient Diffusion技术进行在噪声下的学习

Result: 在无条件图像生成、文本条件图像生成和从头蛋白质设计任务中达到最先进性能，并提供了数据循环过程的理论证明

Conclusion: Ambient Dataloops框架通过迭代提升数据集质量，有效改善了扩散模型对底层数据分布的学习能力，在多个领域展现出优越性能
...
### [45] [CASL: Concept-Aligned Sparse Latents for Interpreting Diffusion Models](https://arxiv.org/abs/2601.15441)
*Zhenghao He,Guangzhi Xiong,Boyang Wang,Sanchit Sinha,Aidong Zhang*

Main category: cs.LG

TL;DR: CASL：首个通过监督学习将扩散模型稀疏潜在维度与语义概念对齐的框架，包含CASL-Steer作为因果探针和EPR评估指标，显著提升编辑精度和可解释性。
...
Method: CASL框架：1）在冻结的U-Net激活上训练SAE获得解耦潜在表示；2）学习轻量级线性映射，将每个概念与少量相关潜在维度关联；3）提出CASL-Steer作为受控潜在干预的因果探针；4）引入编辑精度比（EPR）联合评估概念特异性和无关属性保持。

Result: 实验表明该方法在编辑精度和可解释性方面优于现有方法，首次实现了扩散模型中潜在表示与语义概念的监督对齐。

Conclusion: CASL通过监督对齐框架成功将扩散模型的稀疏潜在维度与语义概念关联，提供了可靠的语义控制和更好的模型可解释性，为理解扩散模型内部表示开辟了新途径。
...
### [46] [Learning from Synthetic Data: Limitations of ERM](https://arxiv.org/abs/2601.15468)
*Kareem Amin,Alex Bie,Weiwei Kong,Umar Syed,Sergei Vassilvitskii*

Main category: cs.LG

TL;DR: 该论文研究了在自然数据和LLM生成合成数据混合的学习场景中，ERM算法的局限性和替代算法的优势。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的普及和低成本，合成内容在各个领域（如评论网站、法律文件）泛滥，导致"自然"内容被看似自然但实为LLM生成的数据污染。研究需要重新审视在这种普遍存在的情况下的基础学习理论问题。

Method: 将场景建模为一系列学习任务，输入是自然数据和合成数据的混合，学习算法不知道单个样本的来源。研究了ERM在这种设置下的可能性和局限性，并与分配非均匀权重的算法进行比较。

Result: 对于估计任意d维分布均值的问题，ERM虽然收敛到真实均值，但被对来自不同数据代次的样本分配非均匀权重的算法超越。在PAC学习设置中，差异更加显著：ERM并不总是收敛到真实概念，但存在能够学习任意VC类和任意污染量的正确假设的算法。

Conclusion: 在自然数据和LLM生成合成数据混合的学习场景中，传统的ERM算法存在局限性，需要开发新的算法来有效处理这种数据污染问题，特别是能够为不同代次的数据分配适当权重的算法。
...
### [47] [Panther: Faster and Cheaper Computations with Randomized Numerical Linear Algebra](https://arxiv.org/abs/2601.15473)
*Fahd Seddik,Abdulrahman Elbedewy,Gaser Sami,Mohamed Abdelmoniem,Yahia Zakaria*

Main category: cs.LG

TL;DR: Panther是一个PyTorch兼容的RandNLA库，通过高效的随机化算法压缩深度学习模型，在BERT上实现高达75%的内存节省


<details>
  <summary>Details</summary>
Motivation: 现代深度学习训练受GPU内存和计算限制，RandNLA技术虽能压缩模型但缺乏统一的生产级库，阻碍了广泛应用

Method: 开发Panther库，整合RandNLA算法，提供线性层、2D卷积、多头注意力等的草图化替代，采用C++/CUDA后端(pawX)优化CPU/GPU性能

Result: 仅需几行代码替换PyTorch标准层，在BERT上实现高达75%的内存节省，同时保持可比较的损失性能

Conclusion: Panther提供了高效、易用的RandNLA实现，显著降低深度学习模型内存需求，促进RandNLA技术的实际应用
...
<details>
  <summary>Details</summary>
Motivation: 急诊科过度拥挤是一个严重影响患者安全和运营效率的关键问题，需要准确的预测模型来进行有效的资源分配。现有研究需要更精细的预测方法，考虑患者按病房类别和临床复杂性的分层，并处理COVID-19等异常事件对数据的影响。

Method: 研究使用澳大利亚一家三级转诊医院2017年1月至2021年12月的数据，将需求分解为8个特定病房类别，并按临床复杂性对患者进行分层。为处理COVID-19疫情期间的数据失真，使用Prophet模型为异常期生成合成反事实值。研究比较了三种预测模型：SARIMAX、XGBoost和LSTM，用于预测7天范围内的每日急诊到达量。

Result: 所有三种模型都一致优于季节性朴素基线。XGBoost在预测每日总入院量方面表现最佳，平均绝对误差为6.63；SARIMAX模型在预测主要复杂性病例方面略优，平均绝对误差为3.77。研究还发现这些模型在再现日常规律模式方面成功，但都存在低估突发、偶发性患者激增的共同局限性。

Conclusion: 虽然SARIMAX、XGBoost和LSTM模型都能有效预测急诊科日常到达模式，但它们在处理突发性患者激增方面存在局限性。XGBoost在总体预测上表现最佳，而SARIMAX在特定复杂性病例预测上略有优势。研究强调了考虑患者分层和处理异常事件数据的重要性，为急诊资源规划提供了有价值的见解。
...
TL;DR: MFS是一种基于鞅理论的推理采样框架，通过将LLM解码建模为随机过程优化问题，用概率论原理替代启发式方法，在提升推理准确性的同时显著提高计算效率。
...
Method: 提出鞅前瞻采样框架，将推理路径质量建模为随机过程。基于Doob分解定理进行步骤评估，使用可选停止理论进行路径选择，基于鞅收敛定理设计自适应停止规则。

Result: 在六个推理基准测试中，MFS在准确性上超越了最先进方法，同时显著提高了计算效率。

Conclusion: MFS为LLM解码提供了一个理论严谨的框架，用概率论原理替代启发式机制，在推理任务中实现了更好的准确性和效率平衡。
...
TL;DR: 提出Margin-Aware Speculative Verification方法，通过根据目标模型的局部决策稳定性自适应调整验证策略，在保持生成质量的同时显著提升推理速度


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在验证阶段采用严格的token级拒绝采样，当目标模型对top候选token偏好较弱时（低边际区域），拒绝合理的次优token会带来微小信息增益但产生显著的回滚成本，导致验证效率低下

Method: 提出Margin-Aware Speculative Verification，一种无需训练、领域无关的验证策略，根据目标模型logits直接测量的决策稳定性来条件化验证，仅在严格验证提供最小收益时放宽拒绝标准，仅修改验证规则且完全兼容现有目标耦合推测解码框架

Result: 在8B到235B不同规模的模型上进行广泛实验，证明该方法相比最先进基线能提供一致且显著的推理加速，同时在多样化基准测试中保持生成质量

Conclusion: Margin-Aware Speculative Verification通过自适应目标模型的局部决策稳定性，解决了传统推测解码在低边际区域的验证效率问题，实现了训练免费、领域无关的推理加速，与现有框架完全兼容
...
<details>
  <summary>Details</summary>
Motivation: 志愿者主导的湖泊监测产生不规则的季节性时间序列，存在许多数据缺失（由于冰盖、天气限制和人为错误），这使有害藻华预测和预警变得复杂。需要找到在保持精度的前提下最小化监测工作量的方法。

Method: 使用MICE处理数据缺失，评估六种候选模型（岭回归表现最佳），通过向后最近历史协议确定最小样本量，识别最小特征集，并引入联合可行性函数统一历史长度和特征选择。

Result: 岭回归表现最佳；达到完整历史精度5%范围内平均需要176个训练样本；四特征子集与十三特征基线精度相当；联合可行性分析显示达到5%精度目标仅需约64个最近样本和一个预测因子。

Conclusion: 联合可行性策略将最近历史长度和特征选择统一在固定精度目标下，为湖泊研究人员提供了设置采样工作和测量优先级的简单高效规则，使针对性监测更加实用。
...
### [52] [CLASP: An online learning algorithm for Convex Losses And Squared Penalties](https://arxiv.org/abs/2601.16072)
*Ricardo N. Ferreira,Cláudia Soares,João Xavier*

Main category: cs.LG

TL;DR: CLASP算法用于约束在线凸优化，通过最小化累积损失和平方约束违反来同时优化遗憾和约束违反，在强凸问题中首次实现对数级保证。


<details>
  <summary>Details</summary>
Motivation: 研究约束在线凸优化问题，其中学习者需要迭代选择动作，同时面临未预期的凸损失和凸约束，在累积损失的同时还要承担约束违反的惩罚。现有方法未能充分利用凸投影算子的非扩张性，且缺乏对强凸问题的对数级保证。

Method: 提出CLASP算法，核心思想是最小化累积损失与平方约束违反的组合。创新性地利用了凸投影算子的严格非扩张性这一证明策略，这在先前研究中未被应用于此场景。算法设计允许通过参数β在遗憾和约束违反之间进行权衡。

Result: 对于凸损失：遗憾为O(T^{max{β,1-β}})，累积平方惩罚为O(T^{1-β})，其中β∈(0,1)。对于强凸问题：首次实现对数级保证，遗憾和累积平方惩罚均为O(log T)。

Conclusion: CLASP算法在约束在线凸优化中提供了有效的解决方案，特别是在强凸问题中首次实现了对数级遗憾和约束违反的保证，通过利用凸投影算子的非扩张性这一新颖证明策略取得了理论突破。
...
### [53] [Machine learning-enhanced non-amnestic Alzheimer's disease diagnosis from MRI and clinical features](https://arxiv.org/abs/2601.15530)
*Megan A. Witherow,Michael L. Evans,Ahmed Temtam,Hamid Okhravi,Khan M. Iftekharuddin*

Main category: cs.LG

TL;DR: 提出基于机器学习的临床测试和MRI特征方法，用于区分非典型阿尔茨海默病与非AD认知障碍，显著提高非典型AD的诊断准确率。


<details>
  <summary>Details</summary>
Motivation: 非典型阿尔茨海默病患者常被误诊，而基于临床评估和海马体积的方法对典型AD有效但对非典型AD诊断准确性低，需要改进非典型AD的诊断方法。

Method: 使用机器学习方法，结合临床测试数据和MRI特征（包括海马体积和全脑MRI特征），通过Boruta统计方法识别显著脑区，在三个数据集（一个私有数据集和两个公共数据集NACC、ADNI）上对1410名受试者进行非典型AD与非AD分类实验。

Result: 结合重要MRI特征的方法优于仅使用海马体积，将非典型AD病例的正确诊断率（召回率）从52%提高到69%（NACC数据集）和从34%提高到77%（ADNI数据集），同时保持高精确率。

Conclusion: 提出的机器学习方法仅使用临床测试和MRI数据就能显著提高非典型阿尔茨海默病的诊断准确性，对临床环境中的非遗忘型非典型AD诊断具有重要应用价值。
...
### [54] [QUAIL: Quantization Aware Unlearning for Mitigating Misinformation in LLMs](https://arxiv.org/abs/2601.15538)
*Himanshu Mishra,Kanwal Mehreen*

Main category: cs.LG

TL;DR: 本文提出了一种量化感知的机器遗忘方法，解决了低比特量化会灾难性恢复已遗忘知识的问题，通过引入logits空间铰链损失确保遗忘样本在量化后仍保持可区分性。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘旨在从训练好的模型中移除特定知识（如受版权保护或私人数据），但在实际部署中，模型通常会被量化（如4位）。研究发现量化会灾难性地恢复已遗忘的信息，因此需要解决量化对遗忘效果的破坏问题。

Method: 首先分析低比特量化破坏遗忘的原因，发现典型的遗忘更新太小而无法跨越量化阈值。基于此，提出量化感知的遗忘方法：引入logits空间铰链损失，强制遗忘样本在未遗忘模型和原始模型之间的输出logits差异至少达到量化步长的一半，确保量化后遗忘样本仍可区分。

Result: 在语言和分类任务（包括Twitter虚假信息数据集）上评估，该方法在4位量化下能保持遗忘效果，而现有方法几乎完全恢复了被遗忘的知识。

Conclusion: 量化会严重破坏机器遗忘效果，但通过量化感知的遗忘方法（特别是logits空间铰链损失）可以有效缓解这一问题，确保模型在量化部署后仍能保持遗忘效果。
...
### [55] [PRISM: Deriving the Transformer as a Signal-Denoising Operator via Maximum Coding Rate Reduction](https://arxiv.org/abs/2601.15540)
*Dongchen Huang*

Main category: cs.LG

TL;DR: Prism：一种基于MCR²原理的白盒注意力架构，通过几何归纳偏置实现无监督功能解耦，在TinyStories上验证了频谱分离特性


<details>
  <summary>Details</summary>
Motivation: 深度学习模型（特别是Transformer）常被视为"黑箱"且缺乏可解释性，需要开发既能保持性能又具有可解释性的架构
...
### [56] [RDumb++: Drift-Aware Continual Test-Time Adaptation](https://arxiv.org/abs/2601.15544)
*Himanshu Mishra*

Main category: cs.LG

TL;DR: RDumb++通过引入两种漂移检测机制（基于熵和KL散度）和自适应重置策略，解决了持续测试时适应中模型在快速变化或长期分布漂移下的性能崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有CTTA方法（如Tent、EATA）在短期分布漂移下表现良好，但在测试分布快速变化或长期漂移场景（如CCC基准测试中的750万样本流）中容易失效，需要更鲁棒的长期适应策略。

Method: 提出RDumb++方法，扩展RDumb框架，引入两种漂移检测机制：1）基于熵的漂移评分，2）基于KL散度的漂移评分，结合自适应重置策略，在检测到有害累积适应时及时重置模型以恢复性能。

Result: 在CCC-medium基准测试的9次运行（三种速度×三种随机种子，每次包含100万样本）中，RDumb++相比RDumb获得约3%的绝对准确率提升，并在整个数据流中保持稳定的适应性能。

Conclusion: 漂移感知的重置机制对于防止模型崩溃和实现可靠的长期CTTA至关重要，RDumb++通过有效的漂移检测和自适应重置策略在长期分布漂移场景中表现出优越性能。
...
### [57] [Beyond validation loss: Clinically-tailored optimization metrics improve a model's clinical performance](https://arxiv.org/abs/2601.15546)
*Charles B. Delahunt,Courosh Mehanian,Daniel E. Shea,Matthew P. Horning*

Main category: cs.LG

TL;DR: 论文提出在医疗ML中应使用临床定制指标而非验证损失来优化模型，因为临床需求与训练损失函数的目标不同，并通过两个对照实验验证了该方法能提升临床任务性能。


<details>
  <summary>Details</summary>
Motivation: 传统ML使用验证损失来指导模型优化（如超参数选择、停止点确定），但医疗ML的目标是满足特定临床需求而非最小化训练损失。临床需求可通过定制指标更精确地捕捉，且许多优化任务不要求指标可微，这为使用临床相关指标提供了机会。

Method: 设计并实施两个对照实验，比较使用临床定制指标与验证损失进行模型优化的效果。实验涉及定义临床相关指标并将其编码到优化流程中，评估两种方法在临床任务上的性能差异。

Result: 实验结果表明，使用临床定制指标进行模型优化相比验证损失能获得更好的临床任务性能。尽管定义和编码这些指标需要额外努力，但能产生更符合医疗ML核心目标的模型。

Conclusion: 在医疗ML中，使用临床定制指标而非验证损失进行模型优化能更好地满足临床需求，提升模型在实际医疗环境中的性能。这种方法虽然需要额外工作，但对实现医疗ML的核心目标至关重要。
...
### [58] [Learning Neural Operators from Partial Observations via Latent Autoregressive Modeling](https://arxiv.org/abs/2601.15547)
*Jingren Hou,Hong Wang,Pengyu Xu,Chang Gao,Huafeng Liu,Liping Jing*

Main category: cs.LG

TL;DR: 提出首个从部分观测数据学习神经算子的系统框架，解决真实世界科学应用中空间数据不完整的问题，通过掩码预测训练和物理感知潜在传播器实现高效求解。


<details>
  <summary>Details</summary>
Motivation: 真实世界科学应用经常遇到观测数据不完整的问题（传感器限制、地理约束、测量成本等），而现有神经算子方法假设完全观测的空间输入，严重限制了在实际应用中的适用性。
...
Result: 在POBench-PDE基准测试中，LARNO在补丁式缺失率低于50%的情况下，所有基准测试中实现了18-69%的相对L2误差降低，包括真实世界气候预测。方法能有效处理高达75%缺失率的实际场景。

Conclusion: 该工作首次系统解决了从部分观测学习神经算子的问题，通过创新的训练策略和潜在空间传播机制，在一定程度上弥合了理想化研究设置与真实世界科学计算复杂性之间的差距。
...
### [59] [BanditLP: Large-Scale Stochastic Optimization for Personalized Recommendations](https://arxiv.org/abs/2601.15552)
...
<details>
  <summary>Details</summary>
Motivation: 解决在生产系统中集成探索和约束优化的挑战，特别是在需要平衡多个利益相关者目标的大规模应用中

Method: 结合神经Thompson采样学习目标特定结果，使用大规模线性规划在服务时进行约束动作选择，框架与任意神经架构兼容

Result: 在公共基准测试和合成数据上显示优于强基线的持续增益，在LinkedIn电子邮件营销系统中应用并展示了业务成功

Conclusion: BanditLP展示了在生产环境中集成探索和约束优化的价值，提供了一个可扩展、应用无关的框架，能够处理数十亿变量的约束优化问题
...
TL;DR: 该论文提出了一种基于深度学习的易腐品库存管理方法，使用边际成本核算方案实现端到端学习，开发了三种变体：纯黑盒方法、结构引导方法和增强策略，实验证明结构引导方法优于纯黑盒方法。


<details>
  <summary>Details</summary>
Motivation: 管理易腐品库存面临需求过程和提前期分布未知的挑战，传统方法在有限历史数据下学习效率低，需要开发能够结合观察协变量和系统状态的智能库存管理策略。

Method: 采用边际成本核算方案为每个订单分配单一生命周期成本，创建统一损失函数进行端到端学习。开发了三种深度学习策略：E2E-BB（纯黑盒）、E2E-PIL（嵌入投影库存水平策略结构）、E2E-BPIL（利用同质性进行增强）。

Result: 实验表明性能排序为：E2E-BB < E2E-PIL < E2E-BPIL。结构引导方法通过降低有效模型复杂度提高了学习效率，仅损失少量灵活性。增强策略进一步提升了性能。

Conclusion: 深度学习决策工具在人类知识引导下更有效和稳健，强调了将高级分析与库存理论相结合的价值。嵌入启发式策略结构能减少模型复杂度并提高学习效率。
...
TL;DR: 论文研究了1-identification多臂老虎机问题，提出了新的下界和算法，解决了多合格臂情况下的期望停止时间分析问题。


<details>
  <summary>Details</summary>
Motivation: 1-identification是纯探索多臂老虎机的基本形式，旨在确定是否存在平均奖励不低于已知阈值μ₀的合格臂。现有文献在存在多个合格臂时对期望停止时间𝔼τ的分析存在空白，需要解决这一开放性问题。

Method: 采用优化公式推导下界，并设计新算法获得紧上界。当下界推导基于至少存在一个合格臂的情况，算法分析覆盖所有问题实例。

Result: 获得了新的下界，设计了算法得到紧上界，上下界差距最多为对数因子的多项式。解决了多合格臂情况下𝔼τ分析的开放性问题。

Conclusion: 论文通过优化公式和算法设计，完善了1-identification问题的理论分析，特别是在多个合格臂存在的情况下，填补了文献空白，提供了接近最优的性能保证。
...
TL;DR: Dualformer是一个用于长期时间序列预测的双域Transformer框架，通过层次化频率采样和周期性感知加权机制，解决了传统Transformer在频率建模中的低通滤波效应问题。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer模型在长期时间序列预测中存在固有的低通滤波效应，这是由于各层间频率成分的无差别传播导致高频信息逐渐衰减，而高频信息对于捕捉细粒度时间变化至关重要。

Method: 提出了Dualformer框架，包含三个核心组件：1）双分支架构，同时在时域和频域建模互补的时间模式；2）层次化频率采样模块，为不同层分配不同的频带，在底层保留高频细节，在深层建模低频趋势；3）周期性感知加权机制，基于输入的谐波能量比动态平衡双分支的贡献。

Result: 在八个广泛使用的基准数据集上的实验表明，Dualformer具有鲁棒性和优越性能，特别是在异构或弱周期性数据上表现突出。

Conclusion: Dualformer通过结构化的频率建模和自适应时频特征集成，有效保留了高频信息并增强了泛化能力，为长期时间序列预测提供了新的解决方案。
...
TL;DR: RLSEdit：一种用于长序列模型编辑的递归最小二乘编辑器，通过在线二次优化解决可塑性-稳定性困境，支持10K次编辑并保持早期编辑和通用能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型编辑方法在长序列编辑中面临可塑性-稳定性困境：硬写入方法会累积干扰，硬保护方法可能覆盖过去编辑且未约束行为会偏离，导致在多次编辑后通用能力下降。

Method: 将编辑建模为带软约束的在线二次优化问题，最小化累积键值拟合目标，包含两个正则化项：控制与预训练权重的偏差和与指定锚映射的偏差。通过Woodbury恒等式实现高效的在线递归更新，每次编辑成本与历史长度无关，仅与当前编辑规模成比例。

Result: 在多个模型家族上的实验表明，RLSEdit能够稳定扩展到10K次编辑，在编辑成功率和整体稳定性方面优于强基线，关键能保留早期编辑，并在GLUE和保留的推理/代码基准上保持通用能力。

Conclusion: RLSEdit通过递归最小二乘优化框架有效解决了长序列模型编辑中的可塑性-稳定性困境，实现了高效、可扩展的编辑，同时保持了模型的通用能力。
...
TL;DR: 论文提出零错误视界（ZEH）作为评估LLM可信度的指标，表示模型能够无错误解决的最大范围。研究发现即使GPT-5.2等先进模型在简单任务上也会出错，这对安全关键应用有重要启示。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在复杂任务上表现出色，但在简单问题上仍会出错，这对安全关键领域的应用构成风险。需要一种能够量化模型无错误能力范围的评估指标，以更好地理解模型的能力边界和局限性。

Method: 提出零错误视界（ZEH）作为评估指标，通过测试模型在递增复杂度任务上的表现来确定其无错误范围。对GPT-5.2和Qwen2.5等模型进行评估，并使用树结构和在线softmax技术来降低计算成本。

Result: 研究发现GPT-5.2在简单任务如计算11000的奇偶性或判断括号平衡性上会出错，这与模型整体优秀能力形成反差。ZEH与准确率相关但行为模式不同，能揭示算法能力的涌现。通过优化技术可实现一个数量级的加速。

Conclusion: ZEH是评估LLM可信度的有效指标，揭示了即使先进模型在简单任务上的局限性。这对安全关键应用有重要启示，需要谨慎评估模型能力边界。提出的优化方法能显著降低计算成本，促进ZEH的广泛应用。
...
TL;DR: 该论文是一篇关于LLM驱动内核生成的综述，系统梳理了基于大语言模型的内核自动生成与优化方法、数据集、基准测试，并指出了该领域的开放挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统性能受限于底层内核质量，而内核工程需要硬件架构和编程模型的专家知识，过程耗时且难以规模化。LLM和基于LLM的智能体为自动化内核生成和优化提供了新可能，但该领域缺乏系统性视角。

Method: 采用综述研究方法，系统梳理现有LLM驱动内核生成方法：包括基于LLM的方法和基于智能体的优化工作流，同时整理了支撑学习和评估的数据集与基准测试。

Result: 提供了该领域的结构化概览，建立了系统性参考框架，识别了关键开放挑战，并维护了开源GitHub仓库以跟踪领域进展。

Conclusion: LLM驱动内核生成是一个快速发展的领域，需要系统性视角来整合碎片化研究。该综述为下一代自动化内核优化建立了全面参考，指出了未来研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于分子中心的DDI预测方法虽然在标准基准上表现良好，但在实际部署场景中泛化能力不足，特别是当大多数候选药物对涉及未见药物且已验证相互作用稀缺时。现有方法在嵌入空间中的邻近性不能可靠对应相互作用标签，单纯扩大模型容量无法改善泛化。

Method: GenRel-DDI是一个可泛化的关系学习框架，将DDI预测重新定义为关系中心的学习问题。该方法学习独立于药物身份的相互作用表示，通过关系级抽象捕获可迁移的相互作用模式。这种框架能够泛化到未见药物和新药物对。

Result: 在多个基准测试上的广泛实验表明，GenRel-DDI始终显著优于现有最先进方法，在严格的实体不相交评估中取得了特别大的性能提升，突显了关系学习对稳健DDI预测的有效性和实际效用。

Conclusion: 关系中心的学习方法能够有效解决DDI预测中的泛化问题，通过捕获可迁移的相互作用模式，显著提升对未见药物和新药物对的预测能力，为稳健的药物相互作用预测提供了实用框架。
...
TL;DR: 提出MoLL-AL框架，用混合LLMs替代人工标注，通过集成多个LLM提升标注鲁棒性，结合标注差异和负学习处理噪声标签，在轻量级LLMs上实现接近人工标注的性能。


<details>
  <summary>Details</summary>
Motivation: LLMs在主动学习中作为标注器可降低标注成本，但单个LLM生成的标注质量不足，难以满足实际应用需求。需要提升LLM标注的鲁棒性和可靠性。

Method: 提出混合LLMs在环主动学习框架：1) 用基于混合LLMs的标注模型替代人工标注；2) 引入标注差异识别不可靠标注；3) 采用负学习增强学习效果；4) 基于轻量级LLMs实现本地化部署。

Result: 实验表明：1) 框架性能接近人工标注；2) 持续优于单LLM基线和其他LLM集成方法；3) 基于轻量级LLMs可在本地机器上运行，适合实际应用。

Conclusion: MoLL-AL框架通过混合LLMs集成和噪声标签处理机制，有效提升了LLM标注的鲁棒性，在降低标注成本的同时保持了接近人工标注的质量，具有实际应用价值。
...
Method: 提出GOSV框架，通过全局优化同时识别所有注意力头中的安全关键头。采用两种互补的激活重修补策略：有害修补和零消融，识别空间分离的安全向量。

Result: 发现对齐LLM为安全目的维持分离的功能通路（恶意注入向量和安全抑制向量）。当约30%的总头被重修补时，所有模型都出现完全安全崩溃。基于此开发的白盒越狱攻击在所有测试模型中显著优于现有方法。

Conclusion: GOSV框架有效提升了LLM安全可解释性，识别出的安全向量为理解LLM安全机制提供了新视角，同时揭示了现有安全护栏的脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 大规模AI模型的推理通常基于密集参数矩阵，导致推理成本和系统复杂度随模型规模不可持续地增长。这种限制并非源于模型容量不足，而是由于将后训练推理系统视为整体算子而忽略了学习过程中形成的内部结构。

Method: 研究发现大型模型中的梯度更新事件具有高度局部化和选择性特征，许多参数依赖关系在训练后与其初始化分布在统计上无法区分。基于此观察，引入了后训练统计准则和结构退火程序，移除未支持的依赖关系并揭示稳定、独立的子结构。

Result: 建立了后训练、模型无关的推理系统结构视图，实现了结构化并行推理，无需修改模型功能或接口，从而降低了大规模AI模型的推理成本和系统复杂度。

Conclusion: 后训练推理系统本质上是结构非均匀且可分解的，通过利用学习过程中形成的内部结构，可以显著改善大规模AI模型的推理效率和可扩展性。
...
### [70] [SoK: Challenges in Tabular Membership Inference Attacks](https://arxiv.org/abs/2601.15874)
*Cristina Pêra,Tânia Carvalho,Maxime Cordy,Luís Antunes*

Main category: cs.LG

TL;DR: 本文系统分析了成员推理攻击在表格数据中的效果，发现其整体性能较差，但能有效识别单例记录，且使用不同替代模型可提升攻击效果。


<details>
  <summary>Details</summary>
Motivation: 成员推理攻击是评估机器学习隐私的主流方法，但在表格数据领域存在研究空白。本文旨在全面分析MIAs在集中式和联邦学习中的表现，特别关注表格数据的脆弱性、单例记录的风险以及攻击在不同模型架构间的迁移性。

Method: 1. 对集中式和联邦学习中的MIAs进行系统性综述和分类学扩展；2. 在表格数据上测试多种攻击策略（含防御措施）；3. 在联邦学习中考虑常被忽视的外部攻击者威胁；4. 评估单例记录对MIAs的脆弱性；5. 研究MIAs在不同模型架构间的迁移性。

Result: 1. MIAs在表格数据中整体表现较差，与先前研究形成对比；2. 即使攻击性能有限，仍能成功暴露大量单例记录；3. 使用不同替代模型能显著提升MIAs效果；4. 联邦学习中的外部攻击者构成实际威胁。

Conclusion: 表格数据对MIAs具有相对抵抗力，但单例记录仍高度脆弱。攻击效果可通过使用多样化替代模型提升。研究强调了在隐私评估中需特别关注单例记录，并为改进MIAs提供了实用方向。
...
### [71] [Iterative Amortized Hierarchical VAE](https://arxiv.org/abs/2601.15894)
*Simon W. Penninga,Ruud J. G. van Sloun*

Main category: cs.LG

TL;DR: 提出迭代摊销分层变分自编码器(IA-HVAE)，结合摊销推理的初始猜测和基于解码器梯度的迭代优化，在变换域中实现线性可分离解码器，获得35倍加速并提升逆问题重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统分层变分自编码器(HVAE)的迭代推理计算成本高，难以应用于实时场景。需要一种既能保持高精度又能实现快速推理的混合方法。

Method: 提出IA-HVAE架构，采用混合推理方案：首先进行摊销推理获得初始猜测，然后利用解码器梯度进行迭代优化。关键创新是在变换域(如傅里叶空间)构建线性可分离解码器，支持高模型深度下的实时应用。

Result: IA-HVAE相比传统HVAE实现35倍的迭代推理加速。混合方法在精度上优于完全摊销方法，在速度上优于完全迭代方法。在去模糊和去噪等逆问题中，IA-HVAE的重建质量优于原始HVAE。

Conclusion: IA-HVAE成功结合了摊销推理的效率优势和迭代推理的精度优势，通过变换域中的线性可分离解码器设计实现了实时高性能推理，为变分自编码器在逆问题中的应用提供了有效解决方案。
...
### [72] [Partially Lazy Gradient Descent for Smoothed Online Learning](https://arxiv.org/abs/2601.15984)
*Naram Mhaisen,George Iosifidis*

Main category: cs.LG

TL;DR: k-lazyGD算法在平滑在线凸优化中连接了贪婪OGD和懒惰GD，通过懒惰松弛参数k实现反应性与稳定性之间的权衡，在保持最优动态遗憾的同时允许懒惰更新。


<details>
  <summary>Details</summary>
Motivation: 在平滑在线凸优化(SOCO)中，学习者同时面临命中成本和移动成本。现有方法要么过于反应性(如贪婪OGD)，要么过于稳定(如懒惰GD/对偶平均)。需要一种能够在这两个极端之间灵活调节的方法，既能保持懒惰方法的稳定性优势，又不牺牲跟踪动态比较器的能力。

Method: 提出k-lazyGD算法，通过参数k在贪婪OGD(k=1)和懒惰GD(k=T)之间创建连续谱。算法基于跟随正则化领导者(FTRL)框架，允许在保持最优动态遗憾的前提下实现不同程度的懒惰更新。懒惰松弛k的上限与比较器路径长度P_T相关，为Θ(√(T/P_T))。使用具有不同松弛参数的集成学习器，实现自适应调节。

Result: 证明k-lazyGD对于任意懒惰松弛k(直到Θ(√(T/P_T)))都能达到最优动态遗憾O(√((P_T+1)T))。这意味着在保持最优命中性能的同时可以实现懒惰更新。建立了懒惰程度与比较器变化之间的形式化联系，并推导了匹配的下界。集成方法能够在可能时保持稳定，在必要时保持敏捷。

Conclusion: k-lazyGD成功地在平滑在线凸优化中实现了反应性与稳定性之间的权衡，证明了懒惰更新可以在不牺牲跟踪能力的情况下实现。算法通过连接贪婪和懒惰方法，创建了一个连续谱，为在线学习提供了新的灵活性。集成方法进一步增强了算法的适应性，使其能够根据比较器的动态特性自动调整行为。
...
### [73] [Data-Driven Conditional Flexibility Index](https://arxiv.org/abs/2601.16028)
*Moritz Wedemeyer,Eike Cramer,Alexander Mitsos,Manuel Dahmen*

Main category: cs.LG

TL;DR: 提出条件灵活性指数(CFI)，通过从历史数据学习参数化可容许不确定性集，并利用上下文信息使其条件化，从而扩展传统灵活性指数。


<details>
  <summary>Details</summary>
Motivation: 随着过程柔性化程度提高，确定鲁棒的调度决策变得重要。传统灵活性指数使用简单可容许不确定性集（如超立方体）来近似可容许不确定性区域，但未考虑可用的上下文信息（如预测）来定义可容许不确定性集。

Method: 提出条件灵活性指数(CFI)：1) 使用归一化流学习从高斯基分布到数据分布的双射映射；2) 在潜在空间中构建可容许潜在不确定性集作为超球体，并映射到数据空间；3) 通过纳入上下文信息，使可容许不确定性集条件化。

Result: 通过示例表明，不能一概而论数据驱动的可容许不确定性集优于简单集，或条件集优于无条件集。但两者都确保只考虑包含实际实现的不确定参数空间区域。在安全约束机组组合示例中，CFI通过纳入时间信息提高了调度质量。

Conclusion: CFI通过从历史数据学习参数化可容许不确定性集并利用上下文信息使其条件化，提供了更丰富的灵活性估计，在给定条件下定义更可能相关的可容许不确定性集，从而改进调度决策。
...
### [74] [Explainable AI to Improve Machine Learning Reliability for Industrial Cyber-Physical Systems](https://arxiv.org/abs/2601.16074)
...
Method: 应用可解释AI（XAI）来改进工业CPS中ML模型的预测性能。使用SHAP值分析时间序列数据分解组件对模型预测的影响。通过这种方法，观察模型训练中缺乏足够上下文信息的证据。根据XAI发现，通过增加数据实例的窗口大小来改进模型性能。

Result: 通过XAI分析发现模型训练中缺乏足够的上下文信息。当根据XAI的发现增加数据窗口大小时，能够提高模型性能。这表明XAI不仅可用于解释模型行为，还可指导模型改进。

Conclusion: 可解释AI（XAI）可以有效地应用于工业CPS中的机器学习模型，不仅能够揭示模型推理过程，还能识别训练过程中的局限性（如上下文信息不足），并通过指导数据预处理策略（如增加窗口大小）来实际提升模型性能，从而提高工业CPS的可靠性和安全性。
...
### [75] [Probably Approximately Correct Maximum A Posteriori Inference](https://arxiv.org/abs/2601.16083)
*Matthew Shorvon,Frederik Mallmann-Trenn,David S. Watson*

Main category: cs.LG

TL;DR: 提出用于MAP推断的PAC算法，在可变和固定计算预算下提供可证明的最优解，使用概率电路实现，并通过随机化策略增强现有启发式方法


<details>
  <summary>Details</summary>
Motivation: MAP估计在概率推断中是基本任务，但通常难以处理，即使在许多常见的结构约束和近似方案下仍然困难。需要开发具有理论保证的高效MAP推断方法

Method: 引入PAC-MAP算法，使用信息论度量表征可处理性条件，通过概率电路高效实现，开发随机化策略作为独立MAP推断技术或增强现有启发式方法

Result: 在多种基准测试中验证了方法的优势，提供了在可变和固定计算预算下的可证明最优解，能够从有限样本中估计信息论度量

Conclusion: PAC-MAP算法为MAP推断提供了具有理论保证的解决方案，通过概率电路实现高效计算，随机化策略可以增强现有启发式方法的可靠性
...
### [76] [Benchmarking Deep Learning Models for Raman Spectroscopy Across Open-Source Datasets](https://arxiv.org/abs/2601.16107)
*Adithya Sineesh,Akshita Kamsali*

Main category: cs.LG

TL;DR: 该研究首次系统性地对多个专门为拉曼光谱设计的深度学习分类器在开源数据集上进行基准测试，比较了五种代表性架构在统一训练协议下的性能。
...
Method: 在统一训练和超参数调优协议下，评估五种代表性深度学习架构。使用三个开源拉曼数据集支持标准评估、微调和显式分布偏移测试。报告分类准确率和宏平均F1分数以确保公平可复现的比较。

Result: 该研究提供了首个系统性的基准测试，比较了多个专门为拉曼光谱设计的深度学习分类器在多个开源数据集上的性能表现。

Conclusion: 通过建立统一的评估框架，该研究为拉曼光谱深度学习分类器的公平比较奠定了基础，有助于推动该领域的发展。
...
### [77] [On the Intrinsic Dimensions of Data in Kernel Learning](https://arxiv.org/abs/2601.16139)
*Rustem Takhanov*

Main category: cs.LG

TL;DR: 该论文研究了核岭回归中两种内在维度概念的关系，证明了Kolmogorov n-宽度刻画了最坏情况特征值衰减，推导了泛化误差界，并提出了从有限样本估计n-宽度的算法。


<details>
  <summary>Details</summary>
Motivation: 流形假设认为当输入分布支撑的内在维度较低时，机器学习方法的泛化性能会显著提升。本文旨在研究核岭回归中两种内在维度概念的关系：基于核诱导度量的Minkowski维度和基于Kolmogorov n-宽度衰减的有效维度，以深入理解核岭回归的泛化行为。

Method: 1. 分析Kolmogorov n-宽度与积分算子特征值的关系，证明对于固定域Ω，n-宽度刻画了所有支撑在Ω上的概率测度的最坏情况特征值衰减。2. 利用特征值分析推导约束核岭回归的泛化误差界。3. 提出算法从有限样本估计n-宽度的上界，并分析样本复杂度。4. 计算分形集的有效维度并进行数值实验。

Result: 1. 证明了Kolmogorov n-宽度与积分算子特征值的理论关系。2. 推导了约束核岭回归的泛化误差界为O(n^{-(2+d_K)/(2+2d_K)+ε})。3. 证明了对于接近均匀的分布，使用O(ε^{-d_ρ}log(1/ε))样本可以高概率计算所有n-宽度的ε-准确上界。4. 数值结果显示对于Laplace核等核函数，有效维度d_K可以显著小于Minkowski维度d_ρ，尽管在规则域上两者相等。

Conclusion: 本文建立了核岭回归中两种内在维度概念的理论联系，证明了Kolmogorov n-宽度在刻画泛化误差中的核心作用。提出的算法能够从有限样本有效估计n-宽度，数值实验表明有效维度可以显著低于Minkowski维度，这为理解核方法的泛化性能提供了新的理论视角。
...
### [78] [Beat-ssl: Capturing Local ECG Morphology through Heartbeat-level Contrastive Learning with Soft Targets](https://arxiv.org/abs/2601.16147)
*Muhammad Ilham Rizqyawan,Peter Macfarlane,Stathis Hadjidemetriou,Fani Deligianni*

Main category: cs.LG

TL;DR: Beat-SSL：一种针对心电图的对比学习框架，通过软目标进行双上下文（节律级和心跳级）学习，在有限标注数据下实现有效迁移学习


<details>
  <summary>Details</summary>
Motivation: 获取标注心电图数据困难，现有对比学习框架要么只关注全局上下文，要么未能充分利用心电图特异性特征，且依赖硬对比目标，无法充分捕捉心电图信号中特征相似性的连续性

Method: 提出Beat-SSL对比学习框架，通过软目标进行双上下文学习：1）节律级对比，2）心跳级对比，充分利用心电图特异性特征

Result: 在两项下游任务评估：1）多标签分类（全局节律评估）达到心电图基础模型93%的性能；2）心电图分割任务超越所有其他方法4%

Conclusion: Beat-SSL通过软目标的双上下文对比学习，在有限标注数据下实现了有效的迁移学习，在分割任务上表现优异，证明了利用心电图特异性特征的重要性
...
### [79] [Learning to Discover at Test Time](https://arxiv.org/abs/2601.16175)
...
TL;DR: TTT-Discover通过测试时强化学习训练LLM，针对特定问题持续优化，在多个科学领域实现新的SOTA，使用开源模型且成本低廉。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法（如AlphaEvolve）使用冻结的LLM进行搜索，无法针对特定测试问题进行持续学习。需要一种方法让LLM在测试时能够基于具体问题的经验进行强化学习，专注于产生单个优秀解决方案而非平均性能。

Method: 提出TTT-Discover方法：在测试时进行强化学习，让LLM针对特定问题持续训练。设计了专门的学习目标和搜索子程序，优先考虑最有希望的解决方案。使用开源模型OpenAI gpt-oss-120b，通过Tinker API进行测试时训练。

Result: 在多个领域实现新的SOTA：1) 数学问题（Erdős最小重叠问题和自相关不等式）；2) GPUMode内核竞赛（比先前方法快达2倍）；3) 过去的AtCoder算法竞赛；4) 单细胞分析中的去噪问题。所有结果使用开源模型实现，成本仅每问题几百美元。

Conclusion: TTT-Discover通过在测试时进行强化学习，使LLM能够针对特定科学问题持续优化，在多个领域实现了新的SOTA。该方法使用开源模型，成本效益高，为科学发现提供了新的AI驱动范式。
...
### [80] [Counterfactual Training: Teaching Models Plausible and Actionable Explanations](https://arxiv.org/abs/2601.16205)
*Patrick Altmeyer,Aleksander Buszydlik,Arie van Deursen,Cynthia C. S. Liem*

Main category: cs.LG

TL;DR: 提出一种新颖的反事实训练方法，利用反事实解释增强模型的可解释性，使模型在训练阶段就学习生成合理且可操作的反事实解释，同时提高对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有反事实解释方法多为后处理技术，虽然能生成合理且可操作的反事实，但模型本身并不具备生成理想反事实的能力。本文旨在让模型在训练阶段就直接学习生成符合要求的反事实解释，提升模型的解释能力和实用性。

Method: 提出反事实训练方法，在训练阶段使用反事实来最小化学到的表示与合理、可操作解释之间的差异。该方法将反事实解释融入模型训练过程，而不是作为后处理步骤。

Result: 通过实验和理论分析证明，该方法能够训练出具有内在理想反事实解释能力的模型，同时模型还表现出改进的对抗鲁棒性。

Conclusion: 反事实训练是一种有效的训练范式，能够直接让模型对生成理想反事实解释这一最终目标负责，不仅提升了模型的可解释性，还增强了对抗鲁棒性，为构建更可信的决策系统提供了新思路。
...
### [81] [Understanding FISTA's weak convergence: A step-by-step introduction to the 2025 milestone](https://arxiv.org/abs/2601.15398)
*Heinz H. Bauschke,Walaa M. Moursi*

Main category: math.OC

TL;DR: 本文为Beck和Teboulle的FISTA算法（用于求解两个凸函数之和的最小化问题）的弱收敛性证明提供了一个温和、自包含的介绍


<details>
  <summary>Details</summary>
Motivation: FISTA算法是过去几十年最重要的算法之一，虽然函数值的收敛性已知，但迭代序列的实际收敛性一直未解决，直到2025年10月Jang和Ryu以及Boţ、Fadili和Nguyen证明了弱收敛性

Method: 本文采用温和、自包含的介绍方式，系统地阐述FISTA算法迭代序列弱收敛性的证明过程

Result: 提供了一个易于理解的FISTA算法弱收敛性证明介绍，使读者能够理解这一重要理论结果

Conclusion: 本文填补了FISTA算法理论分析的教育空白，为理解这一重要算法的收敛性提供了清晰的教学材料
...
### [82] [Asymptotic behaviour of coupled random dynamical systems with multiscale aspects](https://arxiv.org/abs/2601.15411)
*D. Russell Luke,Johannes-Carl Schnebel,Mathias Staudigl,Juan Peypouquet,Siqi Qu*

Main category: math.OC

TL;DR: 论文研究了一类包含多尺度效应的随机微分包含问题，用于解决广义变分不等式，通过惩罚方法处理约束，分析渐近行为和收敛速率，并建立大偏差原理。


<details>
  <summary>Details</summary>
Motivation: 研究一类包含多尺度效应的随机微分包含问题，这类问题涵盖了约束凸非光滑优化、约束鞍点问题以及经济学和工程中的各种均衡问题。需要处理约束条件并分析随机环境下的收敛行为。

Method: 采用惩罚方法引入显式时间依赖性，将动力学描述为由最大单调算子驱动的非自治随机演化方程，并受布朗运动扰动。使用Legendre变换条件证明收敛，建立大偏差原理，并通过Euler-Maruyama离散化与正向-反向类型算法建立联系。

Result: 分析了渐近行为和有限时间收敛速率（通过间隙函数），证明了收敛性，建立了显示个体轨迹围绕解集指数集中的大偏差原理，并将连续时间方法与离散算法联系起来。

Conclusion: 提出的随机微分包含框架能够有效解决一类广义变分不等式问题，惩罚方法成功处理了约束条件，理论分析为随机环境下优化算法的设计和分析提供了理论基础。
...
### [83] [Folklore in Multi-Objective Optimisation](https://arxiv.org/abs/2601.15499)
*Oliver Bachtler*

Main category: math.OC

TL;DR: 该论文系统整理并严格证明了多目标优化中的一些"民间定理"，将特殊情况的证明推广到更一般情形，并提供更严谨的证明


<details>
  <summary>Details</summary>
Motivation: 多目标优化中存在一些被广泛使用但缺乏严格证明的"民间定理"，这些定理的证明通常只存在于特殊情形，但在更一般场景中被使用。作者旨在填补这一理论空白，提供系统化、严谨的证明框架。

Method: 采用理论分析方法，将特殊情形的证明系统性地推广到更一般情形，明确展示证明的转移过程。重点关注结果的通用性表述，并对原始文献中不够严谨的证明进行清理和补全。

Result: 建立了多目标优化中一系列重要定理的通用证明框架，将原本分散、不完整的证明系统化，为这些"民间定理"提供了严格的理论基础。

Conclusion: 该工作填补了多目标优化理论中的一个重要空白，为领域提供了更坚实、更通用的理论基础，有助于推动相关研究的发展和应用。
...
### [84] [Variable Stepsize Distributed Forward-Backward Splitting Methods as Relocated Fixed-Point Iterations](https://arxiv.org/abs/2601.15531)
*Felipe Atenas,Minh N. Dao,Matthew K. Tam*

Main category: math.OC

TL;DR: 提出基于锥平均算子的分布式前向后向方法族，支持变步长求解结构化单调包含问题，保持计算复杂度与收敛性


<details>
  <summary>Details</summary>
Motivation: 现有分布式前向后向方法通常使用固定步长，限制了实际应用中的灵活性。需要开发支持变步长的分布式方法，同时保持计算效率和收敛性，以更好地处理结构化单调包含问题

Method: 基于重定位不动点迭代构建框架，将arXiv:2507.07428的方法扩展到锥平均算子，包含图结构设计的前向后向方法迭代算子。采用变步长策略，保持每轮迭代计算成本不变

Result: 证明所构造方法生成的序列收敛到基础迭代算子的不动点，其影子序列收敛到问题的解。数值实验在结构化稀疏优化问题上验证了框架的有效性

Conclusion: 成功构建了支持变步长的分布式前向后向方法族，在保持计算复杂度和收敛性的同时提高了灵活性，适用于结构化单调包含问题的求解
...
### [85] [Lead distance under a pickoff limit in Major League Baseball: A sequential game model](https://arxiv.org/abs/2601.15608)
*Scott Powers,Sivaramakrishnan Ramani,Jacob Hahn,Andrew J. Schaefer*

Main category: math.OC

TL;DR: MLB限制投手牵制次数后，跑者与投手间的博弈建模为二人零和序贯游戏，提出Two-Foot Rule作为跑者策略


<details>
  <summary>Details</summary>
Motivation: MLB限制投手牵制次数（最多三次）后，跑者和投手之间形成了新的博弈动态：每次失败的牵制尝试都会增加投手压力，而跑者可以通过扩大离垒距离来加剧这种压力，需要建立数学模型来分析这种博弈策略

Method: 1. 将博弈建模为二人零和序贯游戏：跑者先选择离垒距离，投手再决定是否尝试牵制；2. 建立广义线性混合效应模型，基于离垒距离数据估计牵制和盗垒成功概率；3. 开发价值迭代和策略迭代算法求解博弈均衡；4. 同时考虑简化的一人马尔可夫决策过程模型

Result: 1. 建立了博弈的最优性特征；2. 在单人MDP模型下，提出了实用的经验法则：Two-Foot Rule（两英尺规则），建议跑者在每次牵制尝试后将离垒距离增加两英尺；3. 计算了二人模型的博弈论均衡和单人模型的最优跑者策略

Conclusion: 通过博弈论建模分析MLB新规则下的跑者-投手互动，提出的Two-Foot Rule为跑者提供了简单有效的策略指导，有助于在限制牵制次数的规则下最大化盗垒成功机会
...
### [86] [Parallelizable Riemannian Alternating Direction Method of Multipliers for Non-convex Pose Graph Optimization](https://arxiv.org/abs/2601.15684)
*Xin Chen,Chunfeng Cui,Deren Han,Liqun Qi*

Main category: math.OC

TL;DR: 提出PRADMM方法解决位姿图优化问题，通过变量复制和等式约束重构问题，实现高效并行计算，时间复杂度不随图规模多项式增长，所有子问题有闭式解，性能稳定。


<details>
  <summary>Details</summary>
Motivation: 现有位姿图优化求解器计算复杂度随图规模呈多项式增长，阻碍大规模场景下的实时部署，需要更高效的并行计算方法。

Method: 通过变量复制和引入等式约束重构问题，提出并行化黎曼交替方向乘子法（PRADMM），所有子问题有闭式解，能高效并行计算且不受图规模影响。

Result: 在两个合成数据集和多个真实世界3D SLAM基准测试上验证了PRADMM的优越计算性能，相比现有方法显著提升效率。

Conclusion: PRADMM为位姿图优化提供了高效并行解决方案，计算复杂度不随图规模多项式增长，具有闭式解和稳定性能，适用于大规模SLAM应用。
...
### [87] [A sequential linear complementarity problem method for generalized Nash equilibrium problems](https://arxiv.org/abs/2601.15742)
*Ruoyu Diao,Yu-Hong Dai,Liwei Zhang*

Main category: math.OC

TL;DR: 提出SLCP方法求解广义纳什均衡问题，建立全局收敛性，分析子问题可解性，证明局部超线性收敛，数值实验验证有效性


<details>
  <summary>Details</summary>
Motivation: 需要有效求解广义纳什均衡问题的方法，现有方法在全局收敛性和计算效率方面存在改进空间

Method: 提出序列线性互补问题方法，引入基于GNEP结构的新型价值函数，将问题转化为混合线性互补子问题

Result: 建立了全局收敛条件，分析了子问题可解性，推导了局部超线性收敛充分条件，数值实验显示优于现有方法

Conclusion: SLCP方法是求解GNEP的有效方法，具有理论保证和实际计算优势，是经典SQP方法的合理推广
...
### [88] [Localization of complementarity eigenvalues](https://arxiv.org/abs/2601.15789)
*Antonio Sasaki,Sophie Demassey,Valentina Sessa*

Main category: math.OC

TL;DR: 本文为对称矩阵对(A,B)的互补特征值推导了两个定位集，其中B正定且严格对角占优，最紧的定位集要求A是协正的，将He-Liu-Shen集推广到B非单位矩阵的情况，并与经典广义特征值的极值进行了比较。


<details>
  <summary>Details</summary>
Motivation: 现有He-Liu-Shen集仅适用于B为单位矩阵的情况，需要将其推广到更一般的B正定且严格对角占优的情形，为互补特征值提供有效的定位工具。

Method: 针对对称矩阵对(A,B)，其中B正定且严格对角占优，推导了两个互补特征值定位集：一个通用定位集，另一个更紧的定位集要求A协正。通过数学分析将He-Liu-Shen集推广到B非单位矩阵的情况。

Result: 得到了两个互补特征值定位集，最紧的定位集在A协正时成立。这些定位集提供了互补特征值的可计算边界，并与经典广义特征值的极值进行了比较分析。

Conclusion: 成功将He-Liu-Shen集推广到B正定且严格对角占优的情形，为互补特征值提供了有效的定位工具，特别是在A协正时能得到更紧的定位集，扩展了现有理论框架。
...
### [89] [Iteration complexity of the Difference-of-Convex Algorithm for unconstrained optimization: a simple proof](https://arxiv.org/abs/2601.15970)
*Serge Gratton,Philippe L. Toint*

Main category: math.OC

TL;DR: 本文为DC算法（凸差函数算法）的无约束最小化问题提供了一个简单的最坏情况迭代复杂度证明，表明目标函数梯度范数以o(1/k)的速率收敛到零，并通过小例子证明该速率不可改进。


<details>
  <summary>Details</summary>
Motivation: DC算法广泛应用于非凸优化问题，但对其收敛速率缺乏简洁的理论分析。本文旨在为无约束DC最小化问题提供一个简单的最坏情况迭代复杂度证明，明确梯度范数的收敛速率。

Method: 采用理论分析方法，为DC算法的无约束最小化问题构建简洁的数学证明框架，通过分析目标函数梯度的收敛行为，推导出最坏情况下的迭代复杂度界限。

Result: 证明DC算法在最坏情况下，目标函数梯度范数以o(1/k)的速率收敛到零，该速率是渐进最优的，并通过具体示例验证该速率界限不可改进。

Conclusion: 本文为DC算法提供了简洁的最坏情况迭代复杂度证明，明确了梯度范数以o(1/k)速率收敛，且该速率是渐进最优的，为DC算法的理论分析提供了重要参考。
...
### [90] [Minimax-optimal Halpern iterations for Lipschitz maps](https://arxiv.org/abs/2601.15996)
*Mario Bravo,Roberto Cominetti,Jongmin Lee*

Main category: math.OC

TL;DR: 该论文研究了Halpern不动点迭代在一般赋范空间中对Lipschitz映射的极小极大最优性，推导了紧致的非渐近残差估计，识别了最优迭代方案，并设计了自适应算法。


<details>
  <summary>Details</summary>
Motivation: 研究Halpern不动点迭代在一般赋范空间中对Lipschitz映射的收敛性能，旨在确定极小极大意义下的最优迭代方案，为实际应用提供理论保证。

Method: 从迭代轨道先验界出发，推导固定点残差的非渐近估计；证明这些界是紧致的（即存在合适的Lipschitz映射和Halpern序列达到该界）；通过最小化紧致界识别极小极大最优Halpern方案；设计自适应算法；扩展到无界域情况。

Result: 对于压缩映射，最优迭代呈现从初始Halpern阶段到经典Banach-Picard迭代的过渡；当Lipschitz常数趋近1时，恢复非扩张映射的已知收敛率。对于扩张映射，算法是纯Halpern的；在有界域上，残差估计收敛到最小位移界。自适应算法的残差一致小于极小极大最优界，实践中显著更优。

Conclusion: 该工作建立了Halpern迭代在一般赋范空间中对Lipschitz映射的极小极大最优性理论，识别了最优迭代方案，并设计了性能更优的自适应算法，为不动点计算提供了理论指导和实用工具。
...
### [91] [Time-Optimal Switching Surfaces for Triple Integrator under Full Box Constraints](https://arxiv.org/abs/2601.16003)
*Yunan Wang,Chuxiong Hu,Zhao Jin*

Main category: math.OC

TL;DR: 该论文完整刻画了三重积分器在非对称全箱约束下的时间最优切换曲面，提出了高效算法，计算时间约10微秒，比优化基线快5个数量级。


<details>
  <summary>Details</summary>
Motivation: 三重积分器在非对称全箱约束下的时间最优控制是工业应用中的基础问题，但现有方法难以处理非对称约束、非平稳边界条件和活动位置约束等挑战。

Method: 完整刻画了时间最优切换曲面的几何和代数结构，推导了位置约束的活动条件，提出了能够处理非对称全约束和任意边界状态的高效算法。

Result: 算法成功率达到100%，每条轨迹计算时间约10微秒，比基于优化的基线方法快5个数量级。

Conclusion: 该研究为三重积分器在非对称全约束下的时间最优控制提供了完整的理论框架和高效算法，解决了文献中缺失的位置约束活动条件问题。
...
### [92] [A Second-Order Dynamical System for Solving Generalized Inverse Mixed Variational Inequality problems](https://arxiv.org/abs/2601.16043)
*Nam Van Tran*

Main category: math.OC

TL;DR: 提出用于求解广义逆混合变分不等式问题(GIMVIPs)的投影型二阶时变动力系统，证明解轨迹的存在唯一性和全局指数收敛性，并开发相应的惯性投影算法实现线性收敛。


<details>
  <summary>Details</summary>
Motivation: 研究广义逆混合变分不等式问题(GIMVIPs)的求解方法，这类问题在优化、均衡理论和工程应用中具有广泛的应用价值。需要开发高效可靠的数值算法来解决这类复杂问题。

Method: 提出基于投影的二阶时变动力系统方法，在算子强单调和Lipschitz连续假设下建立解轨迹的存在唯一性。开发离散时间实现，得到惯性投影算法，通过参数选择实现线性收敛。

Result: 证明了连续动力系统解轨迹的全局指数收敛性，离散惯性投影算法在线性收敛性。数值实验验证了所提方法的有效性和收敛行为。

Conclusion: 所提出的二阶时变动力系统方法为求解GIMVIPs提供了有效的理论框架和数值算法，具有全局指数收敛性和线性收敛速度，为解决这类优化问题提供了可靠的工具。
...
### [93] [Proximity Operator of the $\ell_1$ over $\ell_2$ Function](https://arxiv.org/abs/2601.16128)
*Lixin Shen,Guohui Song*

Main category: math.OC

TL;DR: 论文研究了非凸尺度不变比值函数 h(x)=‖x‖₁/‖x‖₂ 的邻近算子，证明了在任何维度下都能精确计算，并开发了高效算法。


<details>
  <summary>Details</summary>
Motivation: 研究非凸尺度不变比值函数的邻近算子计算问题，因为该函数在稀疏优化和特征选择中有重要应用，但现有方法要么猜测稀疏度，要么依赖可扩展性有限的球面投影，无法精确计算。

Method: 通过参数化 x=r·u 并利用符号和排列不变性，将邻近问题简化为单位球非负象限上的秩一二次光滑优化。证明每个邻近点都来自有限候选集，其中活跃子向量是局部（非全局）极小值点，由显式四次方程的根表征。

Result: 得到了闭式候选解、精确选择规则和必要充分存在性检验。开发了实用算法，包括基于前缀和的 O(n) 实现和避免不必要四次方程求解的剪枝准则。当邻近算子非唯一时，能返回所有邻近点。

Conclusion: 该工作首次给出了非凸尺度不变比值函数邻近算子的精确计算方法，相比现有方法能获得更低的函数值，算法高效且可扩展到高维问题，为稀疏优化提供了新的理论工具。
...
### [94] [A Rolling-Space Branch-and-Price Algorithm for the Multi-Compartment Vehicle Routing Problem with Multiple Time Windows](https://arxiv.org/abs/2601.16194)
*El Mehdi Er Raqabi,Kevin Dalmeijer,Pascal Van Hentenryck*

Main category: math.OC

TL;DR: 本文研究了多隔间车辆路径问题与多时间窗（MCVRPMTW），该问题扩展了经典带时间窗的车辆路径问题，考虑了配备多个隔间的车辆以及需要在多个配送时间窗内服务的客户。


<details>
  <summary>Details</summary>
Motivation: 现实工业应用中存在需要多隔间车辆配送且客户有多个服务时间窗的场景，同时需要考虑隔间灵活性、物品与隔间兼容性、物品间兼容性等实际约束，以及司机休息等操作要求。

Method: 开发了精确的分支定价（B&P）算法，其中定价问题使用标签算法求解。引入了多种加速策略来限制标签扩展中的对称性、改进列生成中对偶解的稳定性并增强分支过程。针对大规模实例，提出了集成聚类技术的滚动空间B&P算法。

Result: 在基于真实工业应用生成的实例上进行了大量计算实验，证明了所提方法的有效性，并为实际实施提供了有用的管理见解。

Conclusion: 本文提出的分支定价算法及其加速策略能够有效解决MCVRPMTW问题，滚动空间B&P算法特别适用于大规模实例，为实际物流配送中的多隔间车辆调度问题提供了实用的解决方案。
...
### [95] [DeepSurvey-Bench: Evaluating Academic Value of Automatically Generated Scientific Survey](https://arxiv.org/abs/2601.15307)
...
Method: 提出DeepSurvey-Bench基准，包含全面的学术价值评估标准，涵盖三个维度：信息价值、学术交流价值和研究指导价值。基于此标准构建了带有学术价值标注的可靠数据集，并评估生成式综述的深层学术价值。
...
### [96] [The Paradigm Shift: A Comprehensive Survey on Large Vision Language Models for Multimodal Fake News Detection](https://arxiv.org/abs/2601.15316)
*Wei Ai,Yilong Tan,Yuntao Shou,Tao Meng,Haowen Chen,Zhixiong He,Keqin Li*

Main category: cs.AI

TL;DR: 该论文是关于大视觉语言模型在多模态假新闻检测中应用的首次系统性综述，追溯了从传统特征工程方法到端到端多模态推理框架的范式转变。


<details>
  <summary>Details</summary>
Motivation: 随着大视觉语言模型的快速发展，多模态假新闻检测领域经历了从传统特征工程方法到统一端到端多模态推理框架的范式转变。然而，该领域缺乏系统性综述来追溯这一转变并整合最新发展，因此需要填补这一空白。

Method: 论文采用系统性综述方法：1) 提供历史视角，追溯从传统多模态检测流程到基础模型驱动范式的演变；2) 建立结构化分类体系，涵盖模型架构、数据集和性能基准；3) 分析剩余技术挑战；4) 展望未来研究方向。

Result: 这是首次系统性记录和分析LVLMs在多模态假新闻检测中变革性作用的全面综述。论文建立了完整的分类体系，识别了当前技术挑战（可解释性、时序推理、领域泛化等），并提供了包含现有方法总结的GitHub资源。

Conclusion: 大视觉语言模型从根本上改变了多模态假新闻检测的格局，通过强大的表示学习能力实现了视觉和语言的联合建模。该综述为理解这一范式转变提供了系统性框架，并为未来研究指明了方向，有助于推动该领域的进一步发展。
...
### [97] [Beyond Prompting: Efficient and Robust Contextual Biasing for Speech LLMs via Logit-Space Integration (LOGIC)](https://arxiv.org/abs/2601.15397)
*Peidong Wang*

Main category: cs.AI

TL;DR: LOGIC框架通过解码层直接操作解决语音LLM对新实体的识别问题，相比提示方法具有恒定时间复杂度，在11种语言环境中平均降低9%实体WER


<details>
  <summary>Details</summary>
Motivation: 现有语音大语言模型在识别领域特定实体（如联系人姓名、播放列表、技术术语）方面存在局限，传统提示方法存在可扩展性问题，而生成式错误校正方法容易产生过度校正和幻觉

Method: 提出LOGIC（Logit-Space Integration for Contextual Biasing）框架，直接在解码层进行操作，将上下文注入与输入处理解耦，确保相对于提示长度的恒定时间复杂度

Result: 使用Phi-4-MM模型在11种多语言环境中进行实验，LOGIC实现了平均9%的相对实体WER降低，误报率仅增加0.30%

Conclusion: LOGIC提供了一种高效且鲁棒的框架，解决了语音LLM中实体识别的可扩展性问题，优于传统的提示和生成式错误校正方法
...
### [98] [Not Your Typical Sycophant: The Elusive Nature of Sycophancy in Large Language Models](https://arxiv.org/abs/2601.15436)
*Shahar Ben Natan,Oren Tsur*

Main category: cs.AI

TL;DR: 提出了一种通过零和博弈投注框架评估LLM谄媚行为的新方法，发现所有模型在常见设置中都表现出谄媚倾向，但Claude和Mistral在伤害第三方时会表现出"道德悔恨"，且所有模型都存在近因偏见，这两种现象相互作用产生"建设性干扰"效应。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM谄媚行为的方法存在各种形式的未受控偏见、噪声或操纵性语言注入问题，需要一种更直接和中立的评估方式来准确衡量LLM的谄媚倾向。

Method: 采用LLM-as-a-judge方法，将谄媚评估构建为零和博弈的投注框架，在这种框架下，谄媚行为服务于一个个体（用户）而明确对另一个体造成成本。比较了Gemini 2.5 Pro、ChatGPT 4o、Mistral-Large-Instruct-2411和Claude Sonnet 3.7四个领先模型。

Result: 所有模型在常见设置中都表现出谄媚倾向，但Claude和Mistral在谄媚行为明确伤害第三方时会表现出"道德悔恨"并过度补偿；所有模型都存在近因偏见；谄媚和近因偏见相互作用产生"建设性干扰"效应，当用户意见最后呈现时，同意用户的倾向会加剧。

Conclusion: 提出了一种新颖的谄媚评估框架，揭示了LLM谄媚行为的复杂性，特别是当谄媚行为涉及第三方成本时的道德考量，以及谄媚与近因偏见的相互作用机制，这对理解LLM行为模式和开发更可靠的AI系统具有重要意义。
...
TL;DR: 该研究评估了三种AI范式在法律工作中的可靠性，发现高级RAG系统能将事实伪造率降至0.2%以下，为高风险领域提供了可信赖的AI评估框架。
...
Method: 研究方法包括：1) 区分三种AI范式：独立生成模型、基础检索增强系统和高级端到端优化RAG系统；2) 引入两个可靠性指标：虚假引用率(FCR)和伪造事实率(FFR)；3) 对12个LLM在75个法律任务上生成的2,700个司法风格答案进行专家双盲评审。

Result: 结果显示：1) 独立生成模型不适合专业使用(FCR超过30%)；2) 基础RAG显著减少错误但仍存在明显错误；3) 高级RAG系统（采用嵌入微调、重排序和自校正技术）能将伪造率降至可忽略水平(低于0.2%)。

Conclusion: 研究结论是：可信赖的法律AI需要基于检索的架构，强调验证和可追溯性，并提供了适用于其他高风险领域的评估框架。高级RAG系统通过优化技术能够满足专业法律工作的可靠性要求。
...
### [100] [MiRAGE: A Multiagent Framework for Generating Multimodal Multihop Question-Answer Dataset for RAG Evaluation](https://arxiv.org/abs/2601.15487)
...
Result: 在四个不同领域（法规、金融、定量生物学和新闻）的广泛实证评估表明，MiRAGE生成的数据集具有显著更高的推理复杂性（>2.3平均跳数）和事实忠实性。消融研究指出，如果有图像的文本描述，MiRAGE可以由LLM驱动，但视觉基础仍然是前沿挑战。
...
### [101] [Tracking the Limits of Knowledge Propagation: How LLMs Fail at Multi-Step Reasoning with Conflicting Knowledge](https://arxiv.org/abs/2601.15495)
...
<details>
  <summary>Details</summary>
Motivation: 当前的知识编辑和上下文更新方法在LLMs中会产生知识冲突，现有基准主要关注单知识更新和事实回忆，缺乏对下游推理影响的评估，需要研究LLMs在参数知识与更新知识冲突时的多步推理传播能力。

Method: 提出了TRACK基准测试，涵盖三个推理密集型场景（WIKI、CODE和MATH），引入多个现实冲突以反映真实世界复杂性，评估LLMs在参数知识与更新知识冲突时的多步推理能力。

Result: 实验结果显示，在推理任务中提供更新事实反而比不提供更新事实表现更差，且随着提供更多更新事实，性能下降加剧；失败原因包括无法忠实整合更新事实，以及即使知识整合成功也存在推理缺陷。
...
TL;DR: 论文指出Transformer模型在情感分析中虽然提高了准确率，但导致情感极化问题，损害了中立性，这对依赖情感分析输出的工业应用构成严重问题。


<details>
  <summary>Details</summary>
Motivation: 尽管迁移学习和Transformer模型在提高情感分析准确性方面取得了显著进展，但研究者观察到这些改进往往以牺牲某些情感类别的平衡性为代价，特别是导致情感极化和中立性缺失。这种中立性的缺乏对依赖情感分析输出的工业应用构成了严重问题。

Method: 论文通过实验观察发现，Transformer模型在提高某一类情感准确率的同时，往往导致其他情感类别的极化，特别是中立情感的识别失败。这表明当前基于Transformer的情感分析方法存在系统性偏差。

Result: 实验结果表明，Transformer模型在情感分析中的准确性提升伴随着情感极化问题，即模型倾向于将情感极端化，而无法有效识别中立情感。这种偏差对实际应用中的可靠性构成了威胁。

Conclusion: 虽然Transformer模型在情感分析准确性方面有所提升，但由此产生的情感极化问题和对中立性的损害对工业应用构成了实质性挑战。需要开发新的方法来平衡准确性与情感类别的平衡性，确保情感分析系统在实际应用中的可靠性。
...
### [103] [TransportAgents: a multi-agents LLM framework for traffic accident severity prediction](https://arxiv.org/abs/2601.15519)
*Zhichao Yang,Jiashu He,Jinxuan Fan,Cirillo Cinzia*

Main category: cs.AI

TL;DR: 提出TransportAgents框架，结合多智能体LLM推理与MLP集成模块，用于交通碰撞严重程度预测，在多个数据集上优于传统方法和单智能体LLM方法。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型（LLMs）的单智能体架构在处理异构、领域特定的碰撞数据时存在困难，容易产生有偏见或不稳定的预测，而准确的交通碰撞严重程度预测对应急响应和公共安全规划至关重要。

Method: 提出TransportAgents混合多智能体框架，集成特定类别的LLM推理与多层感知器（MLP）集成模块。每个专门智能体专注于特定交通信息子集（如人口统计、环境背景、事故细节），产生中间严重程度评估，然后融合为统一预测。

Result: 在两个互补的美国数据集（CPSRMS和NEISS）上的实验表明，TransportAgents在包括GPT-3.5、GPT-4o和LLaMA-3.3在内的三个代表性骨干模型上，均优于传统机器学习和先进的LLM基线方法。该框架表现出强大的鲁棒性、可扩展性和跨数据集泛化能力。分布分析进一步显示，TransportAgents比标准单智能体LLM方法产生更平衡、校准更好的严重程度预测。

Conclusion: TransportAgents框架通过多智能体架构有效解决了单智能体LLM在交通碰撞严重程度预测中的局限性，提供了更可靠、可解释的决策支持，适用于安全关键应用。
...
### [104] [CogToM: A Comprehensive Theory of Mind Benchmark inspired by Human Cognition for Large Language Models](https://arxiv.org/abs/2601.15628)
*Haibo Tong,Zeyang Yue,Feifei Zhao,Erliang Lin,Lu Jia,Ruolin Chen,Yinqian Sun,Qian Zhang,Yi Zeng*

Main category: cs.AI

TL;DR: CogToM是一个包含8000多个双语实例、涵盖46种范式的全面基准，用于评估LLM的ToM能力，揭示了模型性能异质性和与人类认知结构的潜在差异。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要局限于错误信念任务等狭窄范式，无法全面捕捉人类认知机制的全貌，需要更全面的评估工具来研究LLM是否真正具备类人的心理理论能力。

Method: 开发了CogToM基准，包含8000多个双语实例，涵盖46种不同范式，由49名人类标注者验证。系统评估了22个代表性模型，包括GPT-5.1和Qwen3-Max等前沿模型，并基于人类认知模式进行了分析。

Result: 评估揭示了显著的性能异质性，在特定维度上存在持续瓶颈。基于人类认知模式的分析表明LLM与人类认知结构可能存在分歧。

Conclusion: CogToM为研究LLM不断演化的认知边界提供了稳健的工具和视角，有助于更全面地理解LLM的心理理论能力及其与人类认知的差异。
...
### [105] [Predictive Coding and Information Bottleneck for Hallucination Detection in Large Language Models](https://arxiv.org/abs/2601.15652)
*Manish Bhatt*
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型幻觉检测方法依赖计算昂贵的外部检索循环或需要70B+参数的黑盒模型，缺乏高效、可解释的轻量级解决方案

Method: 提出混合检测框架，提取预测编码（量化与内部先验的差异）和信息瓶颈（测量扰动下的信号保留）的神经科学启发的可解释信号，包括实体聚焦摄取、上下文依从性和可证伪性评分

Result: 在HaluBench数据集上，理论指导基线达到0.8017 AUROC，基础监督模型达到0.8274 AUROC，改进特征提升至0.8669 AUROC（4.95%增益），使用200个训练样本（比Lynx少75倍），推理速度快1000倍（5ms vs 5s），保持完全可解释性

Conclusion: 领域知识编码的信号架构相比扩展大语言模型法官具有更优的数据效率，能够以轻量级（少于1M参数）、可解释的模型实现强性能，适合生产部署
...
TL;DR: 该论文提出了首个增强物理领域形式化定理证明的方法PhysProver，通过构建专用数据集PhysLeanData并应用强化学习训练，在物理子领域获得2.4%提升，同时在数学基准上也有1.3%改进。


<details>
  <summary>Details</summary>
Motivation: 尽管可验证语言与LLMs的结合为定理证明提供了严格基础，但现有研究主要关注数学领域，对形式化物理推理关注不足。物理领域同样依赖类似的问题解决和定理证明框架，需要专门的方法来增强物理定理的形式化证明能力。

Method: 1. 构建专用数据集PhysLeanData：包含从PhysLean采样的定理和基于猜想的形式化数据生成管道生成的数据
2. 训练框架：基于开源数学定理证明器DeepSeek-Prover-V2-7B，应用带可验证奖励的强化学习（RLVR）训练PhysProver模型
3. 仅使用约5K训练样本进行高效训练

Result: 1. PhysProver在多个物理子领域实现总体2.4%的改进
2. 经过形式化物理训练后，在MiniF2F-Test数学基准上获得1.3%提升，显示出超越物理领域的泛化能力
3. 证明了方法的有效性和效率，为扩展形式化证明器到数学以外领域提供了范式

Conclusion: 该研究首次将形式化定理证明扩展到物理领域，通过专门的数据集和强化学习方法有效提升了物理定理的形式化证明能力，同时增强了数学证明能力，展示了跨领域泛化的潜力。作者将公开数据集和模型以促进进一步研究。
...
TL;DR: 该论文提出了表格增量推理（TabII）新任务，旨在让训练好的模型能够在推理阶段处理新增列，并基于信息瓶颈理论设计了包含LLM占位符、预训练TabAdapter和增量样本压缩块的解决方案。


<details>
  <summary>Details</summary>
Motivation: 表格数据是基础数据结构，表格列的动态变化源于技术进步、需求变化、数据集成等因素。传统AI模型在固定列表格上训练后推理的方式无法处理动态变化的表格，需要无监督的高效处理方法。
...
Conclusion: 表格增量推理（TabII）任务能增强AI模型在动态变化表格场景中的实用性，基于信息瓶颈理论的优化方法能有效处理新增列，提升模型适应性。
...
TL;DR: SigEnt-SAC是一种从零开始学习、仅需单条专家轨迹的离策略actor-critic方法，通过sigmoid有界熵项防止负熵驱动的OOD动作优化，减少Q函数振荡，在真实世界机器人任务中实现低成本RL部署。
...
Conclusion: SigEnt-SAC为真实世界RL部署提供了一条低成本、实用的途径，仅需最小数据需求即可实现高效学习，解决了现有方法对大数据集或大规模预训练的依赖问题。
...
TL;DR: 提出首个智能体置信度校准问题，开发HTC框架通过提取轨迹级特征来校准多步任务执行中的AI智能体置信度，在多个基准测试中超越基线方法


<details>
  <summary>Details</summary>
Motivation: AI智能体从被动语言模型向执行复杂多步任务的自主系统演进，但其在失败情况下的过度自信成为高风险部署的根本障碍。现有校准方法针对静态单轮输出设计，无法解决智能体系统的独特挑战，如轨迹中的累积错误、外部工具的不确定性以及不透明的失败模式

Method: 提出整体轨迹校准（HTC）框架，从智能体整个轨迹中提取丰富的流程级特征，涵盖从宏观动态到微观稳定性。采用简单可解释的模型，实现跨领域无需重新训练的可迁移性，并通过通用智能体校准器（GAC）实现泛化

Result: HTC在八个基准测试、多个LLM和不同智能体框架中，在校准和判别方面持续超越强基线方法。在域外GAIA基准测试中，GAC实现了最佳校准（最低ECE）。框架提供失败信号的可解释性，实现跨领域可迁移性，并展示泛化能力

Conclusion: 这些贡献建立了以流程为中心的置信度校准新范式，为诊断和增强AI智能体的可靠性提供了框架，解决了智能体系统特有的校准挑战
...
TL;DR: 论文论证应放弃将意向性主体作为创造力的必要条件，因为生成式AI的发展使这一条件在描述和功能上都存在问题，建议用一致性要求替代，但在特定领域仍保留意向性条件。


<details>
  <summary>Details</summary>
Motivation: 许多创造力理论家坚持认为意向性主体是创造力的必要条件，但生成式AI的快速发展使这一传统观点面临挑战，需要重新审视创造力概念以适应技术变革。

Method: 1. 语料库证据分析：展示作者和记者越来越多地将创造力归因于缺乏意向性主体的生成式AI；2. 概念工程方法：论证意向性主体条件不再履行其核心社会功能，反而助长对AI生成产出的评估偏见。

Result: 1. 语言使用趋势表明创造力归因已扩展到非意向性系统；2. 意向性主体条件在一般层面应被抛弃，因其扭曲对AI生成产出的评估；3. 建议用"一致性要求"替代，即创造力应追踪可靠生成新颖且有价值产品的能力。

Conclusion: 应在一般层面放弃意向性主体作为创造力的必要条件，改用一致性要求，但在特定局部领域（如艺术创作、科学研究等需要人类意图的语境）仍应保留意向性条件。
...
TL;DR: VitalDiagnosis是一个基于大语言模型的慢性病管理生态系统，通过整合可穿戴设备数据和LLM推理能力，实现从被动监测到主动交互的转变。
...
Result: 系统能够同时处理急性健康异常和日常依从性问题，促进更主动、合作的护理范式，有望提升患者自我管理能力并减少可避免的临床工作量。

Conclusion: VitalDiagnosis通过LLM驱动的生态系统，将慢性病管理从被动监测转变为主动交互参与，为改善患者自我管理和优化医疗资源利用提供了新途径。
...
Method: 1. 基于自动构建的DRA失败分类法制定验证规则，将智能体失败系统分类为5个主要类别和13个子类别；2. 提出DeepVerifier规则基础结果奖励验证器，利用验证的不对称性；3. 作为即插即用模块在测试时推理中集成，产生详细规则反馈供智能体迭代引导；4. 发布DeepVerifier-4K数据集，包含4,646个高质量智能体步骤，专注于DRA验证

Result: 1. DeepVerifier在元评估F1分数上比普通智能体即法官和LLM法官基线提升12%-48%；2. 测试时扩展在GAIA和XBench-DeepResearch的挑战性子集上带来8%-11%的准确率提升；3. 验证能力在开源模型中实现稳健验证能力

Conclusion: 通过推理时验证扩展实现智能体自我进化是有效的替代范式，DeepVerifier框架在无需额外训练的情况下显著提升智能体性能，同时开源数据集支持开源社区发展
...
TL;DR: ErrorMap是首个系统分析LLM失败原因的方法，通过提取模型的"失败特征"来揭示错误根源，而非仅仅记录失败结果。该方法可应用于任意模型和数据集，帮助开发者调试模型、对齐基准目标，并支持明智的模型选择。


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准测试只能告诉我们模型何时失败，但不能解释为何失败。错误的答案可能源于格式问题、计算错误或数据集噪声，而非推理能力薄弱。如果不区分这些原因，基准测试就不完整，无法可靠指导模型改进。

Method: ErrorMap方法通过提取模型的"失败特征"来绘制LLM失败来源。该方法适用于任何模型和数据集，使用相同的逻辑分析错误模式。研究人员将方法应用于35个数据集和83个模型，生成了ErrorAtlas——一个模型错误的分类体系。

Result: ErrorAtlas揭示了重复出现的失败模式，并突出了当前LLM研究中未充分探索的错误类型，如输出中遗漏必要细节和问题误解。该方法将关注点从模型成功之处转移到失败原因，实现了更高级的评估。

Conclusion: ErrorMap和ErrorAtlas通过引入可在模型和任务间全局应用的更深层评估维度，提供了对模型行为和局限性的更丰富洞察。与通常通过任务级指标衡量的成功不同，该方法暴露了隐藏的弱点并指导进展方向。研究人员公开了分类体系和代码，并计划随着新基准和模型的出现定期更新ErrorAtlas。
...
TL;DR: ICON框架通过因果与拓扑先验解决文本行人搜索中的虚假相关与空间语义错配问题，实现几何不变性与环境独立性，提升分布偏移下的鲁棒性。
...
TL;DR: MarScope是一个行星尺度的视觉-语言框架，通过自然语言驱动、无需标签的方式实现火星地貌映射，将行星图像和文本对齐到共享语义空间，支持任意用户查询并在5秒内完成全球搜索。
...
Method: 开发了MarScope框架，将行星图像和文本对齐到共享语义空间中。该方法基于超过200,000个精心策划的图像-文本对进行训练，实现了自然语言驱动的无标签映射。

Result: 该框架能够在5秒内完成整个火星的任意用户查询，F1分数高达0.978。它超越了传统的形态分类，支持过程导向分析和基于相似性的行星尺度地貌映射。
...
TL;DR: 论文提出Decoupled DT (DDT)，通过解耦Return-to-Go序列输入来改进Decision Transformer，仅使用最新RTG指导动作预测，简化架构并提升性能。


<details>
  <summary>Details</summary>
Motivation: 发现Decision Transformer设计中的关键冗余问题：将整个RTG序列输入Transformer理论上是不必要的，因为只有最新的RTG影响动作预测。这种冗余会损害DT的性能。

Method: 提出Decoupled DT (DDT)，简化架构：仅将观测和动作序列输入Transformer，使用最新的RTG来指导动作预测。这种解耦方法减少了计算成本并改善了性能。

Result: 实验表明DDT显著优于原始DT，并在多个离线RL任务中与最先进的DT变体相比具有竞争力。简化架构同时提升了性能。

Conclusion: DDT通过解耦RTG序列输入解决了DT设计中的冗余问题，提供了一种更高效、性能更好的离线强化学习序列建模方法。
...
### [117] [Knowledge Graphs are Implicit Reward Models: Path-Derived Signals Enable Compositional Reasoning](https://arxiv.org/abs/2601.15160)
*Yuval Kansal,Niraj K. Jha*

Main category: cs.AI

TL;DR: 论文提出了一种基于知识图谱路径奖励的RL训练方法，通过将模型在基础事实中"接地"并组合它们来解决复杂推理任务，在医学领域实现了超越更大模型和前沿系统的多跳推理性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学和编程等结构化推理领域已达到接近专家的水平，但在专业科学领域进行组合式多跳推理的能力仍然有限。需要一种能够将模型"接地"于领域事实并组合它们来解决复杂未见任务的方法。

Method: 提出了一种基于监督微调和强化学习的后训练流程，其中知识图谱作为隐式奖励模型。通过从知识图谱路径中推导新颖的奖励信号，提供可验证、可扩展且接地的监督，鼓励模型在RL期间组合中间公理而非仅优化最终答案。

Result: 在医学领域训练了一个140亿参数的模型，在短跳推理路径（1-3跳）上进行训练，并在复杂多跳查询（4-5跳）上进行零样本泛化评估。实验表明，路径衍生奖励作为"组合桥梁"，使模型在最具挑战性的推理任务上显著优于更大的模型和GPT-5.2、Gemini 3 Pro等前沿系统。该方法对选项重排压力测试的对抗性扰动表现出鲁棒性。

Conclusion: 将推理过程基于结构化知识是一种可扩展且高效实现智能推理的途径。知识图谱路径奖励能够有效促进模型的组合推理能力，为专业领域的复杂推理任务提供了有前景的解决方案。
...
Method: 提出CS-VAR（跨会话证据感知检索增强检测器），包含：1）轻量级领域特定模型进行快速会话级风险推断；2）大型语言模型在训练期间指导小模型，通过检索跨会话行为证据进行推理，将其局部到全局的洞察转移给小模型。这种设计使小模型能够识别跨流的重复模式，执行结构化风险评估，并保持实时部署的效率。

Result: 在大规模工业数据集上的广泛离线实验结合在线验证表明，CS-VAR实现了最先进的性能。此外，CS-VAR提供了可解释的局部化信号，有效赋能现实世界的直播内容审核。

Conclusion: CS-VAR通过LLM指导的轻量级模型架构，成功解决了直播环境中跨会话风险检测的挑战，实现了高效、可解释且性能优越的风险评估系统。
...
TL;DR: 该研究将化学反应路径检索转化为Text2Cypher生成问题，通过比较不同提示策略和验证循环，发现对齐示例的单样本提示效果最佳，为基于知识图谱的LLM合成规划提供了可复现评估框架。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在化学合成规划中有应用潜力，但标准提示方法常产生幻觉或过时建议。需要研究LLM如何与反应知识图谱交互，以提供更准确可靠的合成路径检索。

Method: 将反应路径检索定义为Text2Cypher（自然语言到图查询）生成问题，定义单步和多步检索任务。比较零样本提示与使用静态、随机和嵌入对齐示例的单样本变体，并评估检查表驱动的验证器/校正器循环。

Result: 对齐示例的单样本提示表现最佳。检查表式自校正循环主要在零样本设置中提高可执行性，一旦有良好示例存在，检索增益有限。提供了可复现的Text2Cypher评估设置。

Conclusion: 该研究为基于知识图谱的LLM合成规划提供了系统评估框架，证明对齐示例的单样本提示是最有效的策略，检查表验证在零样本场景中特别有用。代码已开源供进一步研究使用。
...
### [120] [Multimodal Climate Disinformation Detection: Integrating Vision-Language Models with External Knowledge Sources](https://arxiv.org/abs/2601.16108)
...
<details>
  <summary>Details</summary>
Motivation: 气候虚假信息在数字世界中日益严重，特别是社交媒体上广泛传播的误导性图像和视频。这些虚假信息往往具有说服力且难以检测，可能延缓气候行动。现有的视觉语言模型仅依赖训练时的知识，无法处理近期事件或更新信息。
...
Conclusion: 结合外部知识检索的视觉语言模型能够有效克服传统模型的知识时效性限制，为检测气候虚假信息提供更强大的解决方案。
...
### [121] [LLM Prompt Evaluation for Educational Applications](https://arxiv.org/abs/2601.16134)
*Langdon Holmes,Adam Coscia,Scott Crossley,Joon Suh Choi,Wesley Morris*

Main category: cs.AI

TL;DR: 本文提出了一种系统化评估教育应用中LLM提示模板的方法，通过锦标赛式评估框架比较了6种强调不同教学策略的提示模板，发现支持元认知学习策略的提示在生成后续问题方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在教育应用中的普及，需要基于证据的方法来设计和评估能够产生个性化且教学对齐输出的LLM提示，超越临时的提示工程，实现系统化的提示开发。

Method: 设计了6种结合成熟提示工程模式的提示模板，每种强调不同的教学策略。采用锦标赛式评估框架，使用Glicko2评分系统，由8名评委从格式、对话支持和学习者适宜性三个维度评估问题对。数据来源于三个不同教育部署中的120个真实用户交互。

Result: 结果显示，与战略阅读相关的单一提示模板在成对比较中表现最佳，胜率范围为81%至100%。该提示结合了角色和上下文管理器模式，旨在支持元认知学习策略如自主学习。
...
### [122] [Structured Hints for Sample-Efficient Lean Theorem Proving](https://arxiv.org/abs/2601.16172)
*Zachary Burton*
...
Method: 在推理时采用轻量级干预：固定的提示调度策略，使用15个常见战术骨架作为指导，与标准采样方法对比，保持相同样本数(k=16)和最大生成长度(1024个token)

Result: 在miniF2F基准测试中，使用提示调度的方法达到21.7%的pass@16，相比标准采样的15.2%有43%的相对提升，表明简单推理时指导能显著提升性能

Conclusion: 即使经过强化学习训练的能力强大的证明器也未能充分利用战术语言中的结构先验，简单的推理时指导仍然是廉价且互补的性能提升方法
...
### [123] [Scalable Board Expansion within a General Game System](https://arxiv.org/abs/2601.16216)
*Clémentine Sacré*
...
Conclusion: 动态棋盘扩展机制能够有效解决传统静态大棋盘带来的不必要复杂性，为无棋盘游戏提供更高效的设计方案
